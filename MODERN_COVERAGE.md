...
| ~~2~~ | ~~Hidetsugu's Second Rite~~ | — | **Implemented.** See named-factory table above. |
| ~~3~~ | ~~Faithless Salvaging~~ | — | **Implemented.** See named-factory table above. |
...
| ~~6~~ | ~~Necrodominance~~ | — | **Implemented.** See named-factory table above. |
| ~~7~~ | ~~Phlage, Titan of Fire's Fury~~ | — | **Implemented.** See named-factory table above. ETB damage+life ships; Escape alt-cost deferred (same gap as Uro). |
...
| ~~9~~ | ~~Nadu, Winged Wisdom~~ | — | **Implemented.** See named-factory table above. |
| ~~10~~ | ~~Ajani, Nacatl Pariah~~ | ~~medium~~ | **Shipped** — `AjaniNacatlPariahFactory` (DFC front face). Creature — Cat {1}{W} 1/1 Vigilance + end-step "may sac another → transform" trigger flipping the attached `MdfcState` (CR 711 / CR 701.28). Two 1/1 Cat token rider + back-face PW loyalty abilities + Layer 0 per-face hot-swap deferred (see factory xmldoc). |
...
| ~~15~~ | ~~Karn, Scion of Urza~~ | ~~medium~~ | **Shipped** — `KarnScionOfUrzaFactory`. Legendary Planeswalker — Karn {4} loyalty 5. +1 reveals the top two cards and collapses the opponent's pile-separator + controller's pick-pile prompts to a deterministic split (higher-mv card to hand, other to bottom of library; ties → first card to hand) — same auto-pick posture as the rest of the planeswalker family (Wrenn and Realmbreaker / Liliana of the Veil). -1 is DEFERRED to a no-op body (loyalty change still applies per CR 606.3) pending "exiled with this source" tag tracking on the exile zone (same queue as `KarnTheGreatCreatorFactory`'s wishboard exile reach and `LurrusOfTheDreamDenFactory`'s graveyard fence). -2 creates a 0/0 colorless Construct artifact creature token via `TokenFactory.CreateOnBattlefield` with `CardType.Artifact` additively stamped (CR 301.1 / 302.1 — same multi-type pattern as Wurmcoil Engine's Phyrexian Wurm tokens and Esika's Chariot's vehicle shell); the printed "This creature gets +1/+1 for each artifact you control" rider wires as a per-token `CdaPowerToughnessEffect` (CR 613.7a) registered on the supplied `ContinuousEffectsService` — evaluators read the controller's battlefield at compute time so the token's P/T tracks artifact count dynamically (Tarmogoyf-style CDA path; count includes the token itself since it's stamped Artifact). Single-arg dispatcher path produces the card shape with no live `ContinuousEffectsService` (token is created as a 0/0 — a same-turn SBA 704.5f pass will graveyard it); use the `(owner, zoneService, effects)` overload for fully-wired behavior. Token colour-set deferred (same gap as Wurmcoil Engine + Crashing Footfalls — empty mana cost already implies colourless). Modern affinity / Tron threat. |
...
| ~~17~~ | ~~The Meathook Massacre~~ | — | **Implemented.** See named-factory table above. |
