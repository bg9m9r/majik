...
internal static class DirectDamageRecognizer
{
    ...
    // Referenced (NOT frozen): SharedCardData is plumbing — a lazy holder for
    // the embedded card seed — not bot policy.
    public static int DamageToPlayer(ICard card) =>
        Cache.GetOrAdd(card.Name, static name => Scan(Majik.Bot.Search.SharedCardData.Repo.GetByName(name)?.OracleText));
