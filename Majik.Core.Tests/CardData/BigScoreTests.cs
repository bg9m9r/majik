...
/// <summary>
/// Tests for <see cref="BigScoreFactory"/>.
///
/// Big Score (Outlaws of Thunder Junction, {3}{R}):
///   Instant. As an additional cost to cast this spell, discard a card.
///   Draw two cards and create two Treasure tokens.
///
/// Covers:
///   - Card identity (Instant, {3}{R}, owner / controller).
...
/// resolve-side discard): see <see cref="BigScoreFactory"/>'s XML docs.
/// v1 ships the discard at resolve, so this suite exercises resolve-side
/// discard behaviour rather than the cast-time additional-cost gate.
/// </summary>
public class BigScoreTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void BigScore_Identity()
    {
        var c = BigScoreFactory.Create(_alice);

        c.Name.Should().Be("Big Score");
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_BigScore()
    {
        var card = NamedCardFactory.Create("Big Score", _alice);

        card.Should().BeOfType<Instant>();
        card.Name.Should().Be("Big Score");
    ...
    public void Resolve_DiscardsOne_DrawsTwo_CreatesTwoTreasures()
    {
        ...
        var effects = BigScoreFactory.BuildResolveEffect(_alice);
        ...
        treasures.Should().HaveCount(2,
            "Big Score creates exactly two Treasure tokens (CR 111.10)");
