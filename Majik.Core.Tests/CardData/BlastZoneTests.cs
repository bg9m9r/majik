...
public class BlastZoneTests
{
    ...
    public void SacSelf_OnProdPath_PublishesPermanentSacrificedEvent()
    {
        ...
        // {3},{T},Sac sweep ability publishes PermanentSacrificedEvent
        // (CR 701.16a).
        ...
        var built = NamedCardFactory.Create("Blast Zone", _alice, effects);
        built.Should().BeOfType<Land>();
        var land = (Land)built;
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        var sac = land.Abilities.OfType<ActivatedAbility>()
