...
public class CatharCommandoTests
{
    ...
    public void SacSelf_OnProdPath_PublishesPermanentSacrificedEvent()
    {
        ...
        // {1},Sac ability publishes PermanentSacrificedEvent (CR 701.16a) —
        // the sacrifice fires regardless of target legality.
        ...
        var built = NamedCardFactory.Create("Cathar Commando", _alice, effects);
        built.Should().BeOfType<Creature>();
        var cc = (Creature)built;
        _alice.Zones.Battlefield.AddCard(cc);
        cc.SetZone(ZoneType.Battlefield);

        var sac = cc.Abilities.OfType<ActivatedAbility>()
        ...
        _alice.Zones.Graveyard.GetCards().Should().Contain(cc);
    }
