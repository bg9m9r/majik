...
/// <summary>
/// Tests for <see cref="CostlyPlunderFactory"/> — Instant {1}{B}.
///
...
/// </summary>
public class CostlyPlunderTests
{
    ...
    {
        var card = CostlyPlunderFactory.Create(_alice);

        card.Name.Should().Be("Costly Plunder");
    ...
    [Fact]
    public void CostlyPlunder_IsBlack()
    {
        var card = CostlyPlunderFactory.Create(_alice);
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_CostlyPlunder()
    {
        var card = NamedCardFactory.Create("Costly Plunder", _alice);
    ...
    {
        var def = CostlyPlunderFactory.BuildSpellDefinition(_alice);

        def.AdditionalCostsOrEmpty.Should().ContainSingle()
            .Which.Should().BeOfType<SacrificeAnArtifactOrCreatureAdditionalCost>(
                "Costly Plunder prints 'As an additional cost to cast this spell, sacrifice an artifact or creature.' (CR 601.2f)");
        def.HasVariableX.Should().BeFalse();
        def.Modes.Should().BeEmpty();
        def.TargetRequests.Should().BeEmpty("Costly Plunder has no targets");
    }
    ...
    public void Resolve_DrawsTwoCards_NoToken()
    {
        ...
        foreach (var e in CostlyPlunderFactory.BuildResolveEffect(_alice)) e.Execute();
        ...
        _alice.Zones.Battlefield.GetCards().Should().BeEmpty(
            "Costly Plunder creates no token — it only draws two cards");
    }
