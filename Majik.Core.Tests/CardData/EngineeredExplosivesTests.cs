...
public class EngineeredExplosivesTests
{
    ...
    public void SacSelf_OnProdPath_PublishesPermanentSacrificedEvent()
    {
        ...
        // {2},Sac sweep ability publishes PermanentSacrificedEvent (CR 701.16a).
        ...
        var built = NamedCardFactory.Create("Engineered Explosives", _alice, effects);
        built.Should().BeOfType<Artifact>();
        var ee = (Artifact)built;
        _alice.Zones.Battlefield.AddCard(ee);
        ee.SetZone(ZoneType.Battlefield);

        var sac = ee.Abilities.OfType<ActivatedAbility>()
