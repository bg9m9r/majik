...
public class ExpeditionMapTests
{
    ...
    public void SacSelf_OnProdPath_PublishesPermanentSacrificedEvent()
    {
        ...
        // self-sacrifice ability publishes PermanentSacrificedEvent (CR 701.16a).
        ...
        var built = NamedCardFactory.Create("Expedition Map", _alice, effects);
        built.Should().BeOfType<Artifact>();
        var map = (Artifact)built;
        _alice.Zones.Battlefield.AddCard(map);
        map.SetZone(ZoneType.Battlefield);

        var sac = map.Abilities.OfType<ActivatedAbility>()
