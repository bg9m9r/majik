...
/// <summary>
/// Tests for <see cref="BotanicalSanctumFactory"/> — the Kaladesh
/// "fast land" Botanical Sanctum (G/U member of the cycle).
///
...
/// - Two mana abilities producing {G} and {U}.
...
public class BotanicalSanctumTests
{
    private static Land MakeWithBus(Player owner, ReplacementBus bus) =>
        BotanicalSanctumFactory.Create(owner, bus);
    ...
    [Fact]
    public void BotanicalSanctum_IsLand_WithCorrectName()
    {
        var alice = new Player("Alice", 20);

        var land = BotanicalSanctumFactory.Create(alice);
    ...
    [Fact]
    public void BotanicalSanctum_IsNotBasic_NotLegendary()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void BotanicalSanctum_Dispatch_ResolvesViaNamedCardFactory()
    {
        var alice = new Player("Alice", 20);

        var card = NamedCardFactory.Create("Botanical Sanctum", alice);
    ...
    [Fact]
    public void BotanicalSanctum_HasTwoColouredManaAbilities_G_and_U()
    {
        var alice = new Player("Alice", 20);
        ...
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("G")),
            "Botanical Sanctum produces {G}");
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("U")),
            "Botanical Sanctum produces {U}");
    }

    [Fact]
    public void BotanicalSanctum_HasNoActivatedOrTriggeredAbilities()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void BotanicalSanctum_EntersUntapped_WhenControllerHasZeroOtherLands()
    {
        ...
        after!.EntersTapped.Should().BeFalse(
            "0 other lands is ≤ 2, so Botanical Sanctum enters untapped");
    }

    [Fact]
    public void BotanicalSanctum_EntersUntapped_WhenControllerHasTwoOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 2; i++)
    ...
    [Fact]
    public void BotanicalSanctum_EntersTapped_WhenControllerHasThreeOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 3; i++)
        ...
        after!.EntersTapped.Should().BeTrue(
            "3 other lands exceeds 2, so Botanical Sanctum enters tapped");
    }

    [Fact]
    public void BotanicalSanctum_PredicateExcludesSelf()
    {
    ...
    [Fact]
    public void BotanicalSanctum_EntersUntapped_WhenOnlyOpponentHasManyLands()
    {
    ...
    [Fact]
    public void BotanicalSanctum_SingleArgDispatch_DoesNotRegisterReplacement()
    {
        ...
        var alice = new Player("Alice", 20);
        var land = NamedCardFactory.Create("Botanical Sanctum", alice);
    ...
    [Fact]
    public void BotanicalSanctum_Create_ThrowsOnNullOwner()
    {
        var act = () => BotanicalSanctumFactory.Create(null!);
