...
/// <summary>
/// Tests for <see cref="BountifulLandscapeFactory"/> (Streets of New Capenna) —
/// the exact New Capenna twin of <see cref="TwistedLandscapeFactory"/>.
///
...
///   (<see cref="EvolvingWildsFactory"/>) narrowed to basic Forest / Island /
///   Mountain (CR 205.4a — Basic supertype + one of those land subtypes).
/// - Cycling {G}{U}{R} via the shared
...
public class BountifulLandscapeFactoryTests
{
    ...
    {
        var land = (Land)NamedCardFactory.Create("Bountiful Landscape", _alice);
    ...
    public void Activation_FetchesBasicForestTapped_AndSacrifices()
    {
        ...
        // A basic Plains is NOT a legal target (only Forest/Island/Mountain).
        ...
        // Basic Forest fetched to battlefield tapped; off-color Plains untouched.
        _alice.Zones.Battlefield.GetCards().Should().Contain(basicForest);
        basicForest.IsTapped.Should().BeTrue("CR — put onto the battlefield tapped");
        _alice.Zones.Library.GetCards().Should().Contain(basicPlains,
            "Plains is not a Forest/Island/Mountain");
        _alice.Zones.Library.GetCards().Should().NotContain(basicForest);

        // Bountiful Landscape self-sacrificed.
        _alice.Zones.Graveyard.GetCards().Should().Contain(land);
        _alice.Zones.Battlefield.GetCards().Should().NotContain(land);
    }

    [Fact]
    public void Activation_FetchesBasicIsland_AndBasicMountain_AreLegalTargets()
    {
        ...
        basicIsland.IsTapped.Should().BeTrue();
    }
    ...
    public void HasCyclingActivatedAbility_WithManaAndDiscardSelfCosts()
    {
        ...
        manaCost.Green.Should().Be(1, "cycling {G}{U}{R}");
        manaCost.Blue.Should().Be(1, "cycling {G}{U}{R}");
        manaCost.Red.Should().Be(1, "cycling {G}{U}{R}");
    }
    ...
    [Fact]
    public void Cycling_EndToEnd_PaysGURDiscardsSelfDrawsOne_PublishesEvent()
    {
        ...
        var land = BountifulLandscapeFactory.Create(_alice, eventBus: bus);
        _alice.Zones.Hand.AddCard(land);
        land.SetZone(ZoneType.Hand);

        _alice.AddManaToPool(ManaCost.Parse("GUR"));
