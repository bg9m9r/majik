...
/// <summary>
/// Tests for Brute Force (Odyssey, {R}, Instant — +3/+3 EOT).
///
/// Coverage:
...
public class BruteForceFactoryTests
{
    ...
    {
        var bf = BruteForceFactory.Create(_alice);

        bf.Name.Should().Be("Brute Force");
        bf.ManaCost.Should().Be("{R}");
    ...
    {
        var def = BruteForceFactory.BuildDefinition();
    ...
    public async Task CastAndResolve_TargetGetsPlusThreePlusThree()
    {
        ...
        var bf = BruteForceFactory.Create(_alice);
        bf.SetZone(ZoneType.Hand);
        _alice.Zones.Hand.AddCard(bf);
        ...
        await _flow.CastAsync(_alice, bf, BruteForceFactory.BuildDefinition(), agent, ctx);
