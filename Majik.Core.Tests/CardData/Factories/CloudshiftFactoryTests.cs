...
/// <summary>
/// Unit tests for <see cref="CloudshiftFactory"/>.
///
/// Covers:
...
public class CloudshiftFactoryTests
{
    ...
    [Fact]
    public void Cloudshift_IsInstant_AtCostW()
    {
        var c = CloudshiftFactory.Create(_alice);
    ...
    [Fact]
    public void Cloudshift_Definition_HasSingleControllerCreatureTarget()
    {
        var def = CloudshiftFactory.BuildSpellDefinition(_alice);
    ...
    [Fact]
    public void Cloudshift_Resolve_ExilesThenReturnsTargetCreature()
    {
        ...
        bear.Zone.Should().Be(ZoneType.Battlefield,
            "CR 614 — Cloudshift returns the exiled creature in the same resolution");
        ...
        bear.Controller.Should().BeSameAs(_alice,
            "return is 'under its owner's control' — owner is the original caster here");
    }

    [Fact]
    public void Cloudshift_Resolve_OpponentControlledTarget_Fizzles()
    {
        // Bob's creature — Alice cannot target it (CR 608.2b resolution check).
    ...
    [Fact]
    public void Cloudshift_Resolve_NoTargets_NoOp()
    {
        var def = CloudshiftFactory.BuildSpellDefinition(_alice);
        var effects = def.EffectFactory(new ChosenSpellParams(
            ModeIndex: null,
        ...
        effects.Should().BeEmpty("no targets → no effects produced");
    }
