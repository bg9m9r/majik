...
/// <summary>
/// Tests for <see cref="ConcealedCourtyardFactory"/> — the Kaladesh "fast land"
/// Concealed Courtyard.
///
...
/// - Two mana abilities producing {W} and {B}.
...
/// </summary>
[Trait("Color", "C")]
public class ConcealedCourtyardFactoryTests
{
    private static Land MakeWithBus(Player owner, ReplacementBus bus) =>
        ConcealedCourtyardFactory.Create(owner, bus);
    ...
    [Fact]
    public void ConcealedCourtyard_IsLand_WithCorrectName()
    {
        var alice = new Player("Alice", 20);

        var land = ConcealedCourtyardFactory.Create(alice);
    ...
    [Fact]
    public void ConcealedCourtyard_IsNotBasic_NotLegendary()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void ConcealedCourtyard_HasTwoColouredManaAbilities_W_and_B()
    {
        var alice = new Player("Alice", 20);
        ...
        manaAbilities.Should().HaveCount(2, "one ManaAbility per produced colour ({W} and {B})");

        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("W")),
            "Concealed Courtyard produces {W}");
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("B")),
            "Concealed Courtyard produces {B}");
    }

    [Fact]
    public void ConcealedCourtyard_HasNoActivatedOrTriggeredAbilities()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void ConcealedCourtyard_EntersUntapped_WhenControllerHasZeroOtherLands()
    {
        ...
        after!.EntersTapped.Should().BeFalse(
            "0 other lands is ≤ 2, so Concealed Courtyard enters untapped");
    }

    [Fact]
    public void ConcealedCourtyard_EntersUntapped_WhenControllerHasTwoOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 2; i++)
    ...
    [Fact]
    public void ConcealedCourtyard_EntersTapped_WhenControllerHasThreeOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 3; i++)
        ...
        after!.EntersTapped.Should().BeTrue(
            "3 other lands exceeds 2, so Concealed Courtyard enters tapped");
    }

    [Fact]
    public void ConcealedCourtyard_PredicateExcludesSelf()
    {
    ...
    [Fact]
    public void ConcealedCourtyard_EntersUntapped_WhenOnlyOpponentHasManyLands()
    {
    ...
    [Fact]
    public void ConcealedCourtyard_Create_ThrowsOnNullOwner()
    {
        var act = () => ConcealedCourtyardFactory.Create(null!);
