...
/// <summary>
/// Unit tests for <see cref="ConduitPylonsFactory"/> (Edge of Eternities).
///
/// Land — Desert. Oracle text:
///   "When this land enters, surveil 1.
///    {T}: Add {C}.
///    {1}, {T}: Add one mana of any color."
///
/// Colour-fixing twin of Crystal Grotto: identical ability shape, but the ETB
/// peek is surveil 1 (CR 701.42) rather than scry 1, and the type line carries
/// the Desert land subtype. Loaded from the embedded JSON definition via
/// <see cref="Majik.Core.CardData.Definitions.CardDefinitionFactory"/>.
///
/// Covers the card's UNIQUE behaviour:
/// - Identity: Desert land subtype, non-Basic.
/// - {T}: Add {C} — one cost-free colourless mana ability (CR 605.1a).
/// - {1}, {T}: Add one mana of any color — five per-colour ManaAbility slots,
///   each carrying the {1} additional mana cost (six mana abilities total).
/// - One battlefield-active ETB triggered ability that surveils 1.
/// - Surveil-1 fall-back (no agent) puts the peeked card into the graveyard
///   (CR 701.42) — the distinguishing behaviour vs. Crystal Grotto's scry.
/// - The {1} mana cost gates the any-colour abilities (CR 605.1).
/// </summary>
[Trait("Color", "C")]
public class ConduitPylonsFactoryTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void ConduitPylons_Identity_IsNonBasicDesertLand()
    {
        var land = (Land)NamedCardFactory.Create("Conduit Pylons", _alice);
    ...
    [Fact]
    public void ConduitPylons_HasColorlessManaAbility_NoCost()
    {
        var land = (Land)NamedCardFactory.Create("Conduit Pylons", _alice);

        // {T}: Add {C}. {C} parses to one generic-mana pip; the colourless
        // ability has no WUBRG component and must be activatable from an empty
        // pool (no {1} cost rider).
    ...
    [InlineData("R")]
    [InlineData("G")]
    public void ConduitPylons_HasAnyColorManaAbility_PerColor(string color)
    {
        var land = (Land)NamedCardFactory.Create("Conduit Pylons", _alice);
        ...
        land.Abilities.OfType<ManaAbility>().Should().ContainSingle(m =>
            ...
            $"Conduit Pylons can add {{{color}}} via its any-colour mode");
    }

    [Fact]
    public void ConduitPylons_AnyColorAbility_GatedByOneManaCost()
    {
        // CR 605.1 — the {1} is part of the activation cost. With an empty pool
        // the any-colour modes can't be activated; once {1} is in the pool they
        // can. Mirrors the signet / filter-land posture.
        var alice = new Player("Alice", 20);
        var land = (Land)NamedCardFactory.Create("Conduit Pylons", alice);
    ...
    [Fact]
    public void ConduitPylons_EtbTrigger_IsBattlefieldActive()
    {
        var land = (Land)NamedCardFactory.Create("Conduit Pylons", _alice);
    ...
    [Fact]
    public void ConduitPylons_EtbEffect_SurveilsOne_DefaultsTopCardToGraveyard()
    {
        // CR 701.42 — surveil 1 with no registered agent falls back to putting
        // the single peeked card into the graveyard (the all-to-graveyard
        // default). This is the behaviour that distinguishes Conduit Pylons'
        // ETB from Crystal Grotto's scry (which defaults to the bottom).
        var alice = new Player("Alice", 20);
        ...
        // into the graveyard; the previously-second card is now on top.
        alice.Zones.Library.GetCards().Should().Equal(new[] { second });
        alice.Zones.Graveyard.GetCards().Should().Equal(new[] { top });
    }

    [Fact]
    public void ConduitPylons_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
