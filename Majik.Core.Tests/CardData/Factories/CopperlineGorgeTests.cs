...
/// <summary>
/// Tests for <see cref="CopperlineGorgeFactory"/> — the Innistrad "fast land"
/// Copperline Gorge (R/G member of the cycle).
///
/// Oracle (verified against Scryfall):
///   "This land enters tapped unless you control two or fewer other lands.
///    {T}: Add {R} or {G}."
///
/// Mirrors <see cref="SeachromeCoastTests"/>. Covers:
...
public class CopperlineGorgeTests
{
    private static Land MakeWithBus(Player owner, ReplacementBus bus) =>
        CopperlineGorgeFactory.Create(owner, bus);
    ...
    [Fact]
    public void CopperlineGorge_IsLand_WithCorrectName()
    {
        var alice = new Player("Alice", 20);

        var land = CopperlineGorgeFactory.Create(alice);
    ...
    [Fact]
    public void CopperlineGorge_IsNotBasic_NotLegendary()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void CopperlineGorge_Dispatch_ResolvesViaNamedCardFactory()
    {
        var alice = new Player("Alice", 20);

        var card = NamedCardFactory.Create("Copperline Gorge", alice);
    ...
    [Fact]
    public void CopperlineGorge_HasTwoColouredManaAbilities_R_and_G()
    {
        var alice = new Player("Alice", 20);
        ...
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("R")),
            "Copperline Gorge produces {R}");
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("G")),
            "Copperline Gorge produces {G}");
    }

    [Fact]
    public void CopperlineGorge_HasNoActivatedOrTriggeredAbilities()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void CopperlineGorge_EntersUntapped_WhenControllerHasZeroOtherLands()
    {
        ...
        after!.EntersTapped.Should().BeFalse(
            "0 other lands is ≤ 2, so Copperline Gorge enters untapped");
    }

    [Fact]
    public void CopperlineGorge_EntersUntapped_WhenControllerHasTwoOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 2; i++)
    ...
    [Fact]
    public void CopperlineGorge_EntersTapped_WhenControllerHasThreeOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 3; i++)
        ...
        after!.EntersTapped.Should().BeTrue(
            "3 other lands exceeds 2, so Copperline Gorge enters tapped");
    }

    [Fact]
    public void CopperlineGorge_PredicateExcludesSelf()
    {
    ...
    [Fact]
    public void CopperlineGorge_EntersUntapped_WhenOnlyOpponentHasManyLands()
    {
    ...
    [Fact]
    public void CopperlineGorge_SingleArgDispatch_DoesNotRegisterReplacement()
    {
        ...
        var alice = new Player("Alice", 20);
        var land = NamedCardFactory.Create("Copperline Gorge", alice);
    ...
    [Fact]
    public void CopperlineGorge_Create_ThrowsOnNullOwner()
    {
        var act = () => CopperlineGorgeFactory.Create(null!);
