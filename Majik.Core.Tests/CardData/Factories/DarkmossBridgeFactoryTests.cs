...
/// <summary>
/// Unit tests for <see cref="DarkmossBridgeFactory"/> — Darkmoss Bridge
/// (Modern Horizons 3, the BG member of the artifact "Bridge" tapland cycle).
/// Oracle text (verified against Scryfall):
///   "This land enters tapped.
///    Indestructible
///    {T}: Add {B} or {G}."
///
/// Mirrors <see cref="DrossforgeBridgeFactoryTests"/> (its BR sibling) —
/// Artifact Land typing + printed Indestructible keyword +
/// enters-tapped replacement + one mana ability per produced colour.
/// </summary>
[Trait("Color", "C")]
public class DarkmossBridgeFactoryTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void DarkmossBridge_Identity_ArtifactLand_NotBasic()
    {
        var bridge = DarkmossBridgeFactory.Create(_alice);

        bridge.Name.Should().Be("Darkmoss Bridge");
    ...
    [Fact]
    public void DarkmossBridge_HasPrintedIndestructibleKeyword()
    {
        var bridge = DarkmossBridgeFactory.Create(_alice);
    ...
    [Fact]
    public void DarkmossBridge_HasTwoManaAbilities_ProducingBlackAndGreen()
    {
        var bridge = DarkmossBridgeFactory.Create(_alice);
    ...
    [Fact]
    public void DarkmossBridge_ManaAbilityActivation_TapsLandAndProducesBlack()
    {
        var bridge = DarkmossBridgeFactory.Create(_alice);
    ...
    [Fact]
    public void DarkmossBridge_RegistersEntersTappedReplacement_WhenBusSupplied()
    {
        var replacements = new ReplacementBus();
        var bridge = DarkmossBridgeFactory.Create(_alice, replacements);
