...
/// <summary>
/// Unit tests for <see cref="DimirGuildgateFactory"/> — the U/B Guildgate.
/// Oracle text:
...
/// </summary>
public class DimirGuildgateFactoryTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void DimirGuildgate_Dispatch_ReturnsLandWithGateSubtype()
    {
        var card = NamedCardFactory.Create("Dimir Guildgate", _alice);

        card.Should().BeAssignableTo<Land>();
        card.Name.Should().Be("Dimir Guildgate");
        card.HasSubtype(CardSubtype.Gate).Should().BeTrue();
    }

    [Fact]
    public void DimirGuildgate_HasTwoManaAbilities_ProducingBlueAndBlack()
    {
        var land = (Land)NamedCardFactory.Create("Dimir Guildgate", _alice);
        var mana = land.Abilities.OfType<ManaAbility>().ToList();

        mana.Should().HaveCount(2, "Guildgate taps for {U} or {B}");
        mana.Should().Contain(m => m.ManaGenerated.Blue == 1);
        mana.Should().Contain(m => m.ManaGenerated.Black == 1);
    }

    [Fact]
    public void DimirGuildgate_HasNoCyclingAbility()
    {
        var land = (Land)NamedCardFactory.Create("Dimir Guildgate", _alice);
