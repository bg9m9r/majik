...
/// <summary>
/// Unit tests for <see cref="DivinationFactory"/>.
///
/// Oracle text: "Draw two cards." ({2}{U} Sorcery, M10 and many reprints)
///
/// Covers:
/// - Card identity (Sorcery, {2}{U}, blue, CMC 3, owner/controller).
...
/// </summary>
public class DivinationFactoryTests
{
    ...
    [Fact]
    public void Divination_HasSorceryShape_Blue_AtCost2U()
    {
        var card = DivinationFactory.Create(_alice);

        card.Name.Should().Be("Divination");
    ...
    [Fact]
    public void NamedCardFactory_DispatchByName_ReturnsDivinationShape()
    {
        var dispatched = NamedCardFactory.Create("Divination", _alice);

        dispatched.Should().BeOfType<Sorcery>();
        dispatched.Name.Should().Be("Divination");
    ...
    [Fact]
    public void Divination_SpellDefinition_HasNoTargets_NoModes_NoX()
    {
        var def = DivinationFactory.BuildSpellDefinition(_alice);
    ...
    [Fact]
    public void Divination_Resolve_DrawsExactlyTwoCards_LibraryShrinksByTwo()
    {
        ...
        var effect = DivinationFactory.BuildResolveEffect(_alice).Single();
        effect.Execute();
    ...
    [Fact]
    public void Divination_Resolve_EmptyLibrary_FlagsLossSba_DoesNotThrow()
    {
