...
/// <summary>
/// Unit tests for <see cref="ForsakenSanctuaryFactory"/> — Forsaken Sanctuary
/// (common tapped-dual cycle, white/black member). Oracle text:
///   "This land enters tapped.
///    {T}: Add {W} or {B}."
///
/// Mirrors <see cref="HighlandLakeFactoryTests"/>: identity + two mana
/// abilities (one per produced colour {W}/{B}), no extra activated abilities,
/// and the enters-tapped replacement registration (CR 614.1c) when a
/// <see cref="ReplacementBus"/> is supplied.
/// </summary>
[Trait("Color", "C")]
public class ForsakenSanctuaryFactoryTests
{
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void ForsakenSanctuary_HasTwoManaAbilities_ProducingWhiteAndBlack()
    {
        var land = (Land)NamedCardFactory.Create("Forsaken Sanctuary", _alice);
    ...
    [Fact]
    public void ForsakenSanctuary_HasNoExtraActivatedAbilities()
    {
        var land = (Land)NamedCardFactory.Create("Forsaken Sanctuary", _alice);
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void ForsakenSanctuary_RegistersEntersTappedReplacement_WhenBusSupplied()
    {
        var replacements = new ReplacementBus();
        var land = ForsakenSanctuaryFactory.Create(_alice, replacements: replacements);
