...
/// <summary>
/// Tests for <see cref="GrixisPanoramaFactory"/> (Alara Reborn — the Grixis
...
/// - The Bant Panorama tutor-onto-battlefield-tapped idiom narrowed to basic
///   Island / Swamp / Mountain (CR 205.4a), with the extra generic {1} on the
///   fetch cost (CR 117.5).
/// </summary>
[Trait("Color", "C")]
public class GrixisPanoramaFactoryTests
{
    ...
    {
        var land = (Land)NamedCardFactory.Create("Grixis Panorama", _alice);

        land.Name.Should().Be("Grixis Panorama");
    ...
    public void Activation_NoLegalBasic_StillSacrifices()
    {
        ...
        _alice.Zones.Graveyard.GetCards().Should().Contain(land);
        // Nonbasic untouched (only basics are legal AND only I/S/M subtypes).
