...
/// <summary>
/// Tests for <see cref="HauntedRidgeFactory"/> — the Innistrad: Midnight Hunt
/// B/R "slow land" Haunted Ridge.
///
...
/// </summary>
public class HauntedRidgeFactoryTests
{
    private static Land MakeWithBus(Player owner, ReplacementBus bus) =>
        HauntedRidgeFactory.Create(owner, bus);
    ...
    [Fact]
    public void HauntedRidge_IsLand_WithCorrectName()
    {
        var alice = new Player("Alice", 20);

        var land = HauntedRidgeFactory.Create(alice);
    ...
    [Fact]
    public void HauntedRidge_IsNotBasic_NotLegendary()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void HauntedRidge_Dispatch_ResolvesViaNamedCardFactory()
    {
        var alice = new Player("Alice", 20);

        var card = NamedCardFactory.Create("Haunted Ridge", alice);
    ...
    [Fact]
    public void HauntedRidge_HasTwoColouredManaAbilities_B_and_R()
    {
        var alice = new Player("Alice", 20);
        ...
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("B")),
            "Haunted Ridge produces {B}");
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("R")),
            "Haunted Ridge produces {R}");
    }

    [Fact]
    public void HauntedRidge_HasNoActivatedOrTriggeredAbilities()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void HauntedRidge_EntersTapped_WhenControllerHasZeroOtherLands()
    {
        ...
        after!.EntersTapped.Should().BeTrue(
            "0 other lands is < 2, so Haunted Ridge enters tapped");
    }

    [Fact]
    public void HauntedRidge_EntersTapped_WhenControllerHasOneOtherLand()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void HauntedRidge_EntersUntapped_WhenControllerHasTwoOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 2; i++)
    ...
    [Fact]
    public void HauntedRidge_PredicateExcludesSelf()
    {
    ...
    [Fact]
    public void HauntedRidge_EntersTapped_WhenOnlyOpponentHasManyLands()
    {
    ...
    [Fact]
    public void HauntedRidge_SingleArgDispatch_DoesNotRegisterReplacement()
    {
        ...
        var alice = new Player("Alice", 20);
        var land = NamedCardFactory.Create("Haunted Ridge", alice);
    ...
    [Fact]
    public void HauntedRidge_Create_ThrowsOnNullOwner()
    {
        var act = () => HauntedRidgeFactory.Create(null!);
