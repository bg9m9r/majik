...
/// <summary>
/// Tests for Infuriate (Ixalan, {R}, Instant — +3/+2 EOT).
///
/// Coverage:
...
public class InfuriateFactoryTests
{
    ...
    {
        var inf = InfuriateFactory.Create(_alice);

        inf.Name.Should().Be("Infuriate");
        inf.ManaCost.Should().Be("{R}");
        inf.HasType(CardType.Instant).Should().BeTrue();
        inf.Owner.Should().BeSameAs(_alice);
    ...
    {
        var def = InfuriateFactory.BuildDefinition();
    ...
    [Fact]
    public async Task CastAndResolve_TargetGetsPlusThreePlusTwo()
    {
        var bear = BuildBear(_alice);

        var inf = InfuriateFactory.Create(_alice);
        inf.SetZone(ZoneType.Hand);
        _alice.Zones.Hand.AddCard(inf);
        ...
        await _flow.CastAsync(_alice, inf, InfuriateFactory.BuildDefinition(), agent, ctx);
        _resolver.ResolveTop(_stack);

        // 2/2 bear + 3 power + 2 toughness = 5/4
