...
/// <summary>
/// Unit tests for <see cref="MeanderingRiverFactory"/> — Meandering River
/// (common tapped-dual cycle, white/blue member). Oracle text:
///   "This land enters tapped.
///    {T}: Add {W} or {U}."
///
/// Mirrors <see cref="HighlandLakeFactoryTests"/>: identity + two mana
/// abilities (one per produced colour {W}/{U}), no extra activated abilities,
/// and the enters-tapped replacement registration (CR 614.1c) when a
/// <see cref="ReplacementBus"/> is supplied.
/// </summary>
[Trait("Color", "C")]
public class MeanderingRiverFactoryTests
{
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void MeanderingRiver_HasTwoManaAbilities_ProducingWhiteAndBlue()
    {
        var land = (Land)NamedCardFactory.Create("Meandering River", _alice);
    ...
    [Fact]
    public void MeanderingRiver_HasNoExtraActivatedAbilities()
    {
        var land = (Land)NamedCardFactory.Create("Meandering River", _alice);
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void MeanderingRiver_RegistersEntersTappedReplacement_WhenBusSupplied()
    {
        var replacements = new ReplacementBus();
        var land = MeanderingRiverFactory.Create(_alice, replacements: replacements);
