...
/// <summary>
/// Tests for Monstrous Growth ({1}{G}, Sorcery — +4/+4 EOT).
///
/// Coverage:
/// - Card identity (Sorcery, green, {1}{G}, mana value 2, owner/controller wired).
...
/// </summary>
public class MonstrousGrowthFactoryTests
{
    ...
    public MonstrousGrowthFactoryTests()
    {
    ...
    [Fact]
    public void Create_HasSorceryShape_Green_AtCost1G()
    {
        var mg = MonstrousGrowthFactory.Create(_alice);

        mg.Name.Should().Be("Monstrous Growth");
        mg.ManaCost.Should().Be("{1}{G}");
        mg.HasType(CardType.Sorcery).Should().BeTrue();
        mg.Owner.Should().BeSameAs(_alice);
        mg.Controller.Should().BeSameAs(_alice);
    ...
    public void Create_HasManaValueTwo()
    {
        ...
        mg.ManaCostValue.TotalValue.Should().Be(2);
    }

    [Fact]
    public void NamedCardFactory_DispatchByName_ReturnsMonstrousGrowth()
    {
        var dispatched = NamedCardFactory.Create("Monstrous Growth", _alice);

        dispatched.Should().BeOfType<Sorcery>();
        dispatched.Name.Should().Be("Monstrous Growth");
    ...
    {
        var def = MonstrousGrowthFactory.BuildDefinition();
