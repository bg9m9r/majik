...
/// <summary>
/// Tests for <see cref="NayaPanoramaFactory"/> (Conflux — the Naya member of
/// the paid-fetch "Panorama" cycle).
///
...
///   basic Mountain / Forest / Plains (CR 205.4a), with the extra generic {1}
///   on the fetch cost (CR 117.5).
/// </summary>
[Trait("Color", "C")]
public class NayaPanoramaFactoryTests
{
    ...
    {
        var land = (Land)NamedCardFactory.Create("Naya Panorama", _alice);

        land.Name.Should().Be("Naya Panorama");
    ...
    [Fact]
    public void Activation_FetchesBasicMountainTapped_AndSacrifices()
    {
        ...
        // Basic Mountain fetched to battlefield tapped; off-color Island untouched.
    ...
    public void Activation_NoLegalBasic_StillSacrifices()
    {
        ...
        var dual = new Land(
            "Sacred Foundry", supertypes: null,
            new[] { CardSubtype.Mountain, CardSubtype.Plains });
