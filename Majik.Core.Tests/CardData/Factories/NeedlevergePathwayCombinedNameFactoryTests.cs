...
/// <summary>
/// Tests for <see cref="NeedlevergePathwayCombinedNameFactory"/> — the COMBINED
/// printed name "Needleverge Pathway // Pillarverge Pathway" of the Streets of
/// New Capenna modal double-faced land.
///
/// The two single faces ("Needleverge Pathway" front / "Pillarverge Pathway"
...
/// Needleverge Pathway land, back = Pillarverge Pathway land). Both faces are
/// plain lands (CR 712.3 / 712.4 — choose which face to play; faces do not
/// transform).
///
/// Covers:
/// - Combined arm produces the front-face Land (name, type, {R} mana ability).
...
/// </summary>
[Trait("Color", "C")]
public class NeedlevergePathwayCombinedNameFactoryTests
{
    ...
    {
        var card = NeedlevergePathwayCombinedNameFactory.Create(_alice);
    ...
    [Fact]
    public void CombinedArm_HasSingleTapForRedManaAbility()
    {
        var card = NeedlevergePathwayCombinedNameFactory.Create(_alice);
    ...
    public void CombinedArm_CarriesMdfcState_OnFrontFace()
    {
        ...
        card.MdfcState!.FrontFaceName.Should().Be("Needleverge Pathway");
        card.MdfcState!.BackFaceName.Should().Be("Pillarverge Pathway");
        card.MdfcState!.IsBackFace.Should().BeFalse("starts on the front face");
        card.MdfcState!.ActiveFaceName.Should().Be("Needleverge Pathway");
    }
    ...
    public void NamedCardFactory_DispatchesCombinedName_ToFrontFaceLand()
    {
        ...
        ((Land)card).MdfcState!.BackFaceName.Should().Be("Pillarverge Pathway");
    }
