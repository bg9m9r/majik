...
/// <summary>
/// Unit tests for <see cref="OrzhovGuildgateFactory"/> — the W/B Guildgate.
/// Oracle text:
...
/// </summary>
public class OrzhovGuildgateFactoryTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void OrzhovGuildgate_Dispatch_ReturnsLandWithGateSubtype()
    {
        var card = NamedCardFactory.Create("Orzhov Guildgate", _alice);

        card.Should().BeAssignableTo<Land>();
        card.Name.Should().Be("Orzhov Guildgate");
        card.HasSubtype(CardSubtype.Gate).Should().BeTrue();
    }

    [Fact]
    public void OrzhovGuildgate_HasTwoManaAbilities_ProducingWhiteAndBlack()
    {
        var land = (Land)NamedCardFactory.Create("Orzhov Guildgate", _alice);
        var mana = land.Abilities.OfType<ManaAbility>().ToList();

        mana.Should().HaveCount(2, "Guildgate taps for {W} or {B}");
        mana.Should().Contain(m => m.ManaGenerated.White == 1);
        mana.Should().Contain(m => m.ManaGenerated.Black == 1);
    }

    [Fact]
    public void OrzhovGuildgate_HasNoCyclingAbility()
    {
        var land = (Land)NamedCardFactory.Create("Orzhov Guildgate", _alice);
