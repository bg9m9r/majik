...
/// <summary>
/// Unit tests for <see cref="PrairieStreamFactory"/> — Prairie Stream, a Battle
/// for Zendikar "battle land" / "tango land" (Plains Island dual). Oracle text:
///   "({T}: Add {W} or {U}.)
///    This land enters tapped unless you control two or more basic lands."
///
/// Mirrors <see cref="CinderGladeFactoryTests"/> — same battle-land cycle, only
/// the printed subtypes and produced colors differ ({W}/{U} vs {R}/{G}).
///
/// Covers:
/// - Identity (Land + both printed subtypes Plains/Island, non-Basic).
...
/// </summary>
[Trait("Color", "C")]
public class PrairieStreamFactoryTests
{
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void PrairieStream_IsNotBasic()
    {
    ...
    [Fact]
    public void PrairieStream_HasBothPrintedSubtypes()
    {
        var land = PrairieStreamFactory.Create(_alice);
        land.HasSubtype(CardSubtype.Plains).Should().BeTrue("Prairie Stream is a Plains");
        land.HasSubtype(CardSubtype.Island).Should().BeTrue("Prairie Stream is an Island");
    }

    [Fact]
    public void PrairieStream_HasTwoManaAbilities_ProducingWU()
    {
        var land = (Land)NamedCardFactory.Create("Prairie Stream", _alice);
        var mana = land.Abilities.OfType<ManaAbility>().ToList();

        mana.Should().HaveCount(2, "Prairie Stream taps for {W} or {U}");
        mana.Should().Contain(m => m.ManaGenerated.White == 1);
        mana.Should().Contain(m => m.ManaGenerated.Blue == 1);
    }

    [Fact]
    public void PrairieStream_HasNoActivatedOrTriggeredAbilities()
    {
        var land = PrairieStreamFactory.Create(_alice);
    ...
    [Fact]
    public void PrairieStream_EntersTapped_WhenControllerHasNoBasics()
    {
        var bus = new ReplacementBus();
        var land = PrairieStreamFactory.Create(_alice, replacements: bus);
    ...
    [Fact]
    public void PrairieStream_EntersTapped_WhenControllerHasExactlyOneBasic()
    {
        var bus = new ReplacementBus();
    ...
    [Fact]
    public void PrairieStream_EntersUntapped_WhenControllerHasTwoBasics()
    {
        var bus = new ReplacementBus();
        SeedBasic("Plains", _alice);
    ...
    [Fact]
    public void PrairieStream_EntersUntapped_WhenControllerHasMoreThanTwoBasics()
    {
        var bus = new ReplacementBus();
        SeedBasic("Plains", _alice);
        SeedBasic("Island", _alice);
    ...
    [Fact]
    public void PrairieStream_NonbasicLands_DoNotCount()
    {
        // Two nonbasic lands (other Prairie Streams) are NOT basic lands, so
        // the predicate is unmet -> enters tapped. CR 205.4a: only the Basic
        // supertype makes a land "basic".
        var bus = new ReplacementBus();
        var other1 = PrairieStreamFactory.Create(_alice);
        var other2 = PrairieStreamFactory.Create(_alice);
    ...
    [Fact]
    public void PrairieStream_EntersTapped_WhenOnlyOpponentHasBasics()
    {
    ...
    [Fact]
    public void PrairieStream_PredicateExcludesSelf()
    {
        // Even on the battlefield Prairie Stream isn't a basic land, so it
    ...
    [Fact]
    public void PrairieStream_Create_ThrowsOnNullOwner()
    {
        var act = () => PrairieStreamFactory.Create(null!);
