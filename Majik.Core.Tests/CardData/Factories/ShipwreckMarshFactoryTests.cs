...
/// <summary>
/// Tests for <see cref="ShipwreckMarshFactory"/> — the Innistrad: Midnight
/// Hunt U/B "slow land" Shipwreck Marsh.
///
...
///   (CR 614.1c): "two or more other lands" ⇒ tapped at ≤1 other land,
...
/// </summary>
public class ShipwreckMarshFactoryTests
{
    private static Land MakeWithBus(Player owner, ReplacementBus bus) =>
        ShipwreckMarshFactory.Create(owner, bus);
    ...
    [Fact]
    public void ShipwreckMarsh_IsLand_WithCorrectName()
    {
        var alice = new Player("Alice", 20);

        var land = ShipwreckMarshFactory.Create(alice);
    ...
    [Fact]
    public void ShipwreckMarsh_IsNotBasic_NotLegendary()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void ShipwreckMarsh_Dispatch_ResolvesViaNamedCardFactory()
    {
        var alice = new Player("Alice", 20);

        var card = NamedCardFactory.Create("Shipwreck Marsh", alice);
    ...
    [Fact]
    public void ShipwreckMarsh_HasTwoColouredManaAbilities_U_and_B()
    {
        var alice = new Player("Alice", 20);
        ...
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("U")),
            "Shipwreck Marsh produces {U}");
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("B")),
            "Shipwreck Marsh produces {B}");
    }

    [Fact]
    public void ShipwreckMarsh_HasNoActivatedOrTriggeredAbilities()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void ShipwreckMarsh_EntersTapped_WhenControllerHasZeroOtherLands()
    {
        ...
        after!.EntersTapped.Should().BeTrue(
            "0 other lands is < 2, so Shipwreck Marsh enters tapped");
    }

    [Fact]
    public void ShipwreckMarsh_EntersTapped_WhenControllerHasOneOtherLand()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void ShipwreckMarsh_EntersUntapped_WhenControllerHasTwoOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 2; i++)
    ...
    [Fact]
    public void ShipwreckMarsh_PredicateExcludesSelf()
    {
        // "two or more OTHER lands" — the slow land must not count itself.
        // Seed exactly 1 other land, then place the slow land on the
        // battlefield too; the count must stay 1 (< 2 ⇒ tapped). If self
        // were counted the total would be 2 and the card would wrongly
        // enter untapped.
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        var p = (Land)NamedCardFactory.Create("Plains", alice);
    ...
    [Fact]
    public void ShipwreckMarsh_EntersTapped_WhenOnlyOpponentHasManyLands()
    {
    ...
    [Fact]
    public void ShipwreckMarsh_SingleArgDispatch_DoesNotRegisterReplacement()
    {
        ...
        var alice = new Player("Alice", 20);
        var land = NamedCardFactory.Create("Shipwreck Marsh", alice);
    ...
    [Fact]
    public void ShipwreckMarsh_Create_ThrowsOnNullOwner()
    {
        var act = () => ShipwreckMarshFactory.Create(null!);
