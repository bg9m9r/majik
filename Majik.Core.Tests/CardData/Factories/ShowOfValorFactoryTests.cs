...
/// <summary>
/// Tests for Show of Valor (Jumpstart 2022, {1}{W}, Instant — +2/+4 EOT).
///
/// Coverage:
/// - Card identity (Instant, white, {1}{W}, mana value 2, owner/controller wired).
/// - NamedCardFactory dispatcher returns the correct shape.
/// - SpellDefinition shape (1 target creature, no modes, no X).
/// - Cast + resolve: target gets +2/+4.
...
public class ShowOfValorFactoryTests
{
    ...
    public ShowOfValorFactoryTests()
    {
    ...
    {
        var sov = ShowOfValorFactory.Create(_alice);

        sov.Name.Should().Be("Show of Valor");
        sov.ManaCost.Should().Be("{1}{W}");
        sov.HasType(CardType.Instant).Should().BeTrue();
        sov.Owner.Should().BeSameAs(_alice);
        sov.Controller.Should().BeSameAs(_alice);
        CardColors.GetColors(sov).Should().Contain(ManaColor.White);
    }

    [Fact]
    public void Create_HasManaValueTwo()
    {
        var sov = ShowOfValorFactory.Create(_alice);
    ...
    {
        var def = ShowOfValorFactory.BuildDefinition();
    ...
    [Fact]
    public async Task CastAndResolve_TargetGetsPlusTwoPlusFour()
    {
        var bear = BuildBear(_alice);

        var sov = ShowOfValorFactory.Create(_alice);
        sov.SetZone(ZoneType.Hand);
        _alice.Zones.Hand.AddCard(sov);
        ...
        await _flow.CastAsync(_alice, sov, ShowOfValorFactory.BuildDefinition(), agent, ctx);
        _resolver.ResolveTop(_stack);

        bear.GetPower().Should().Be(4);    // 2 base + 2
        bear.GetToughness().Should().Be(6); // 2 base + 4
    }
