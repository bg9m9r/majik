...
/// <summary>
/// Tests for Smelt ({R}, Instant).
///
/// Oracle text:
///   "Destroy target artifact."
///
/// Covers:
///   - Card shape + dispatch ({R}, Red, Instant).
...
public class SmeltFactoryTests
{
    ...
    [Fact]
    public void Smelt_HasInstantShape_Red_AtCostR()
    {
        var card = SmeltFactory.Create(_alice);

        card.Name.Should().Be("Smelt");
    ...
    {
        var def = SmeltFactory.BuildDefinition(o => o);
    ...
    public void DestroysArtifact_MovesToGraveyard()
    {
        ...
        artifact.Zone.Should().Be(ZoneType.Graveyard,
            because: "Smelt destroys target artifact (CR 701.7)");
    }
    ...
    {
        // A creature is not a legal target for Smelt. If somehow resolved
        ...
        creature.Zone.Should().Be(ZoneType.Battlefield,
            because: "Smelt targets artifact only (CR 608.2b)");
    }
    ...
    {
        // An enchantment is not a legal target for Smelt (unlike Disenchant).
        // CR 608.2b → no-op.
        ...
        enchantment.Zone.Should().Be(ZoneType.Battlefield,
            because: "Smelt targets artifacts only, not enchantments (CR 608.2b)");
    }
