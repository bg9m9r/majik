...
/// <summary>
/// Tests for <see cref="SpirebluffCanalFactory"/> — the Kaladesh "fast land"
/// Spirebluff Canal.
///
...
public class SpirebluffCanalFactoryTests
{
    private static Land MakeWithBus(Player owner, ReplacementBus bus) =>
        SpirebluffCanalFactory.Create(owner, bus);
    ...
    [Fact]
    public void SpirebluffCanal_IsLand_WithCorrectName()
    {
        var alice = new Player("Alice", 20);

        var land = SpirebluffCanalFactory.Create(alice);
    ...
    [Fact]
    public void SpirebluffCanal_IsNotBasic_NotLegendary()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void SpirebluffCanal_HasTwoColouredManaAbilities_U_and_R()
    {
        var alice = new Player("Alice", 20);
        ...
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("U")),
            "Spirebluff Canal produces {U}");
        manaAbilities.Should().Contain(m => SameCost(m.ManaGenerated, ManaCost.Parse("R")),
            "Spirebluff Canal produces {R}");
    }

    [Fact]
    public void SpirebluffCanal_HasNoActivatedOrTriggeredAbilities()
    {
        var alice = new Player("Alice", 20);
    ...
    [Fact]
    public void SpirebluffCanal_EntersUntapped_WhenControllerHasZeroOtherLands()
    {
        ...
        after!.EntersTapped.Should().BeFalse(
            "0 other lands is ≤ 2, so Spirebluff Canal enters untapped");
    }

    [Fact]
    public void SpirebluffCanal_EntersUntapped_WhenControllerHasTwoOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 2; i++)
    ...
    [Fact]
    public void SpirebluffCanal_EntersTapped_WhenControllerHasThreeOtherLands()
    {
        var bus = new ReplacementBus();
        var alice = new Player("Alice", 20);
        for (var i = 0; i < 3; i++)
        ...
        after!.EntersTapped.Should().BeTrue(
            "3 other lands exceeds 2, so Spirebluff Canal enters tapped");
    }

    [Fact]
    public void SpirebluffCanal_PredicateExcludesSelf()
    {
    ...
    [Fact]
    public void SpirebluffCanal_EntersUntapped_WhenOnlyOpponentHasManyLands()
    {
    ...
    {
        var act = () => SpirebluffCanalFactory.Create(null!);
