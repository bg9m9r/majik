...
/// <summary>
/// Unit tests for <see cref="StoneQuarryFactory"/> — Stone Quarry (common
/// tapped-dual cycle, red/white member). Oracle text:
///   "This land enters tapped.
///    {T}: Add {R} or {W}."
///
/// Mirrors <see cref="HighlandLakeFactoryTests"/>: identity + two mana
/// abilities (one per produced colour {R}/{W}), no extra activated abilities,
/// and the enters-tapped replacement registration (CR 614.1c) when a
/// <see cref="ReplacementBus"/> is supplied.
/// </summary>
[Trait("Color", "C")]
public class StoneQuarryFactoryTests
{
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void StoneQuarry_HasTwoManaAbilities_ProducingRedAndWhite()
    {
        var land = (Land)NamedCardFactory.Create("Stone Quarry", _alice);
    ...
    [Fact]
    public void StoneQuarry_HasNoExtraActivatedAbilities()
    {
        var land = (Land)NamedCardFactory.Create("Stone Quarry", _alice);
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void StoneQuarry_RegistersEntersTappedReplacement_WhenBusSupplied()
    {
        var replacements = new ReplacementBus();
        var land = StoneQuarryFactory.Create(_alice, replacements: replacements);
