...
/// <summary>
/// Unit tests for <see cref="TempleOfAbandonFactory"/> (Theros Beyond Death).
///
/// R/G "scry land". Oracle text:
///   "This land enters tapped.
///    When this land enters, scry 1. (Look at the top card of your library.
///    You may put that card on the bottom.)
///    {T}: Add {R} or {G}."
///
/// Same oracle shape as <see cref="TempleOfTriumphFactory"/>; only the mana
/// colours differ ({R}/{G} instead of {R}/{W}). The ETB keyword action is
/// scry 1 (CR 701.20). Loaded from the embedded JSON definition via
...
/// </summary>
public class TempleOfAbandonTests
{
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_TempleOfAbandon()
    {
        var card = NamedCardFactory.Create("Temple of Abandon", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Temple of Abandon");
    ...
    [Fact]
    public void TempleOfAbandon_EtbEffect_ScriesOne_DefaultsTopCardToBottom()
    {
    ...
    [Fact]
    public void TempleOfAbandon_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
