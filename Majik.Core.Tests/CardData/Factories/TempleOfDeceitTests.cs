...
/// <summary>
/// Unit tests for <see cref="TempleOfDeceitFactory"/> (Theros).
///
...
///    {T}: Add {U} or {B}."
///
/// Same oracle shape as <see cref="TempleOfTriumphFactory"/>, only the
/// produced colours are {U}/{B} instead of {R}/{W} (CR 605.1a). The ETB
...
/// </summary>
public class TempleOfDeceitTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void TempleOfDeceit_IsLand_WithCorrectName()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Deceit", _alice);

        land.Name.Should().Be("Temple of Deceit");
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_TempleOfDeceit()
    {
        var card = NamedCardFactory.Create("Temple of Deceit", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Temple of Deceit");
        card.Abilities.OfType<ManaAbility>().Should().HaveCount(2);
        card.Abilities.OfType<TriggeredAbility>().Should().HaveCount(1);
    }

    [Fact]
    public void TempleOfDeceit_HasManaAbility_ForBlue()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Deceit", _alice);
    ...
    [Fact]
    public void TempleOfDeceit_HasManaAbility_ForBlack()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Deceit", _alice);
    ...
    [Fact]
    public void TempleOfDeceit_EtbTrigger_IsBattlefieldActive()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Deceit", _alice);
    ...
    [Fact]
    public void TempleOfDeceit_EtbEffect_ScriesOne_DefaultsTopCardToBottom()
    {
        ...
        var land = (Land)NamedCardFactory.Create("Temple of Deceit", alice);
    ...
    [Fact]
    public void TempleOfDeceit_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
