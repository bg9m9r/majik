...
/// <summary>
/// Unit tests for <see cref="TempleOfMaliceFactory"/> (Theros Beyond Death).
///
...
/// produced colours differ ({B}/{R} here vs {R}/{W}). The ETB keyword action
...
/// </summary>
public class TempleOfMaliceTests
{
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_TempleOfMalice()
    {
        var card = NamedCardFactory.Create("Temple of Malice", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Temple of Malice");
    ...
    [Fact]
    public void TempleOfMalice_EtbEffect_ScriesOne_DefaultsTopCardToBottom()
    {
    ...
    [Fact]
    public void TempleOfMalice_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
