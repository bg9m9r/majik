...
///    {T}: Add {G} or {W}."
///
/// Same Theros scry-land shape as <see cref="TempleOfTriumphFactory"/>, only
/// the two mana colours differ ({G}/{W} vs {R}/{W}). The ETB keyword action
...
public class TempleOfPlentyTests
{
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_TempleOfPlenty()
    {
        var card = NamedCardFactory.Create("Temple of Plenty", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Temple of Plenty");
    ...
    [Fact]
    public void TempleOfPlenty_EtbEffect_ScriesOne_DefaultsTopCardToBottom()
    {
    ...
    [Fact]
    public void TempleOfPlenty_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
