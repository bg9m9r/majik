...
/// <summary>
/// Unit tests for <see cref="TempleOfSilenceFactory"/> (Theros).
///
...
/// {W}/{B}. The ETB keyword action is scry 1 (CR 701.20). Loaded from the
...
/// </summary>
public class TempleOfSilenceTests
{
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_TempleOfSilence()
    {
        var card = NamedCardFactory.Create("Temple of Silence", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Temple of Silence");
    ...
    [Fact]
    public void TempleOfSilence_EtbEffect_ScriesOne_DefaultsTopCardToBottom()
    {
    ...
    [Fact]
    public void TempleOfSilence_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
