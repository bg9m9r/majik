...
/// <summary>
/// Unit tests for <see cref="TempleOfTriumphFactory"/> (Theros Beyond Death).
///
...
///    {T}: Add {R} or {W}."
///
/// Same oracle shape as the Murders at Karlov Manor surveil-land cycle
/// (<see cref="CommercialDistrictFactory"/>), only the ETB keyword action
/// is scry 1 (CR 701.20) instead of surveil 1 (CR 701.42). Loaded from the
/// embedded JSON definition via <see cref="Majik.Core.CardData.Definitions.CardDefinitionFactory"/>.
///
/// Covers:
...
/// this named-card factory — same posture as the surveil-land cycle.
/// </summary>
public class TempleOfTriumphTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void TempleOfTriumph_IsLand_WithCorrectName()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Triumph", _alice);

        land.Name.Should().Be("Temple of Triumph");
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_TempleOfTriumph()
    {
        var card = NamedCardFactory.Create("Temple of Triumph", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Temple of Triumph");
        card.Abilities.OfType<ManaAbility>().Should().HaveCount(2);
        card.Abilities.OfType<TriggeredAbility>().Should().HaveCount(1);
    }

    [Fact]
    public void TempleOfTriumph_HasManaAbility_ForRed()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Triumph", _alice);
    ...
    [Fact]
    public void TempleOfTriumph_HasManaAbility_ForWhite()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Triumph", _alice);
    ...
    [Fact]
    public void TempleOfTriumph_EtbTrigger_IsBattlefieldActive()
    {
        var land = (Land)NamedCardFactory.Create("Temple of Triumph", _alice);
    ...
    [Fact]
    public void TempleOfTriumph_EtbEffect_ScriesOne_DefaultsTopCardToBottom()
    {
        ...
        var land = (Land)NamedCardFactory.Create("Temple of Triumph", alice);
    ...
    [Fact]
    public void TempleOfTriumph_EtbEffect_EmptyLibrary_NoOp()
    {
        var alice = new Player("Alice", 20);
