...
/// <summary>
/// Unit tests for <see cref="ThornglintBridgeFactory"/> — Thornglint Bridge
/// (Tarkir: Dragonstorm, GW artifact "Bridge" tapland).
...
public class ThornglintBridgeFactoryTests
{
    ...
    [Fact]
    public void ThornglintBridge_Identity_ArtifactLand_NotBasic()
    {
        var bridge = ThornglintBridgeFactory.Create(_alice);

        bridge.Name.Should().Be("Thornglint Bridge");
    ...
    [Fact]
    public void ThornglintBridge_HasTwoManaAbilities_ProducingGreenAndWhite()
    {
        var bridge = ThornglintBridgeFactory.Create(_alice);
    ...
    [Fact]
    public void ThornglintBridge_ManaAbilityActivation_TapsLandAndProducesGreen()
    {
        var bridge = ThornglintBridgeFactory.Create(_alice);
    ...
    [Fact]
    public void ThornglintBridge_RegistersEntersTappedReplacement_WhenBusSupplied()
    {
        var replacements = new ReplacementBus();
        var bridge = ThornglintBridgeFactory.Create(_alice, replacements);
