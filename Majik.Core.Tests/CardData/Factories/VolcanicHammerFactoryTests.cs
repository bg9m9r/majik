...
/// <summary>
/// Unit tests for <see cref="VolcanicHammerFactory"/> (Ninth Edition / Tenth Edition,
/// {1}{R}).
///
/// Covers:
/// - Identity ({1}{R} Sorcery).
...
public class VolcanicHammerFactoryTests
{
    ...
    [Fact]
    public void VolcanicHammer_Identity_SorceryAt1R()
    {
        var hammer = VolcanicHammerFactory.Create(_alice);

        hammer.Name.Should().Be("Volcanic Hammer");
        hammer.HasType(CardType.Sorcery).Should().BeTrue();
        hammer.ManaCost.ToString().Should().Be("{1}{R}");
    ...
    {
        var def = VolcanicHammerFactory.BuildSpellDefinition(resolver: x => x);
    ...
    [Fact]
    public void VolcanicHammer_Resolve_DealsThreeDamageToPlayer()
    {
        var def = VolcanicHammerFactory.BuildSpellDefinition(resolver: x => x);
        ...
        _bob.LifeTotal.Should().Be(17, "Volcanic Hammer deals 3 damage to any target");
    }

    [Fact]
    public void VolcanicHammer_Resolve_DealsThreeDamageToCreature()
    {
        ...
        wall.Damage.Should().Be(3, "Volcanic Hammer deals 3 damage to target creature");
    }
}
