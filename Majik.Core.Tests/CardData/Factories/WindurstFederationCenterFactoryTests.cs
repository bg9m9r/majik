...
/// <summary>
/// Unit tests for <see cref="WindurstFederationCenterFactory"/> — the G/W "Town"
...
/// </summary>
[Trait("Color", "M")]
public class WindurstFederationCenterFactoryTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void WindurstFederationCenter_HasTwoManaAbilities_ProducingGreenAndWhite()
    {
        var land = (Land)NamedCardFactory.Create("Windurst, Federation Center", _alice);
        var mana = land.Abilities.OfType<ManaAbility>().ToList();

        mana.Should().HaveCount(2, "Windurst taps for {G} or {W}");
        mana.Should().Contain(m => m.ManaGenerated.Green == 1);
        mana.Should().Contain(m => m.ManaGenerated.White == 1);
    }

    [Fact]
    public void WindurstFederationCenter_Identity_IsLandWithTownSubtype()
    {
        var land = (Land)NamedCardFactory.Create("Windurst, Federation Center", _alice);
    ...
    [Fact]
    public void WindurstFederationCenter_HasNoCyclingAbility()
    {
        var land = (Land)NamedCardFactory.Create("Windurst, Federation Center", _alice);

        land.Abilities.OfType<ActivatedAbility>()
            .Should().BeEmpty("Windurst has no cycling clause");
    }
}
