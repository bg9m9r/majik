...
/// <summary>
/// Unit tests for <see cref="WoodlandStreamFactory"/> — Woodland Stream (common
/// tapped-dual cycle, green/blue member). Oracle text:
///   "This land enters tapped.
///    {T}: Add {G} or {U}."
///
/// Mirrors <see cref="HighlandLakeFactoryTests"/>: identity + two mana
/// abilities (one per produced colour {G}/{U}), no extra activated abilities,
/// and the enters-tapped replacement registration (CR 614.1c) when a
/// <see cref="ReplacementBus"/> is supplied.
/// </summary>
[Trait("Color", "C")]
public class WoodlandStreamFactoryTests
{
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void WoodlandStream_HasTwoManaAbilities_ProducingGreenAndBlue()
    {
        var land = (Land)NamedCardFactory.Create("Woodland Stream", _alice);
    ...
    [Fact]
    public void WoodlandStream_HasNoExtraActivatedAbilities()
    {
        var land = (Land)NamedCardFactory.Create("Woodland Stream", _alice);
    ...
    // -----------------------------------------------------------------------
    [Fact]
    public void WoodlandStream_RegistersEntersTappedReplacement_WhenBusSupplied()
    {
        var replacements = new ReplacementBus();
        var land = WoodlandStreamFactory.Create(_alice, replacements: replacements);
