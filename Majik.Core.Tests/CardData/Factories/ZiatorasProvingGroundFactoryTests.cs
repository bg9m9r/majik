...
/// <summary>
/// Unit tests for <see cref="ZiatorasProvingGroundFactory"/> — the B/R/G
/// triome (Swamp Mountain Forest tapland with Cycling {3}).
///
/// Covers:
/// - Identity (Land + the three basic-land subtypes Swamp/Mountain/Forest).
...
/// </summary>
public class ZiatorasProvingGroundFactoryTests
{
    ...
    [Fact]
    public void ZiatorasProvingGround_Dispatch_ReturnsLandWithName()
    {
        var card = NamedCardFactory.Create("Ziatora's Proving Ground", _alice);

        card.Should().BeAssignableTo<Land>();
        card.Name.Should().Be("Ziatora's Proving Ground");
    ...
    public void ZiatorasProvingGround_CarriesBasicLandSubtype(CardSubtype subtype)
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);

        land.HasSubtype(subtype).Should().BeTrue(
            $"the printed type line is 'Land — Swamp Mountain Forest', so it must carry {subtype}");
    }

    [Fact]
    public void ZiatorasProvingGround_OwnerAndControllerAreSet()
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);
    ...
    [Fact]
    public void ZiatorasProvingGround_IsNotLegendaryOrBasic()
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);
    ...
    [Fact]
    public void ZiatorasProvingGround_HasExactlyThreeManaAbilities()
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);
    ...
    [InlineData("R")]
    [InlineData("G")]
    public void ZiatorasProvingGround_HasManaAbilityProducingColor(string color)
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);
    ...
    [Fact]
    public void ZiatorasProvingGround_HasCyclingActivatedAbility_WithGenericThreeAndDiscardSelf()
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);
    ...
    [Fact]
    public void ZiatorasProvingGround_HasCyclingKeywordMarker()
    {
        var land = ZiatorasProvingGroundFactory.Create(_alice);
    ...
    [Fact]
    public void ZiatorasProvingGround_Cycling_EndToEnd_PaysThreeDiscardsSelfDrawsOne()
    {
        ...
        var triome = ZiatorasProvingGroundFactory.Create(_alice, eventBus: bus);
