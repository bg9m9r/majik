...
/// <summary>
/// Tests for <see cref="MishrasFoundryFactory"/> — manland with a {2}
/// animate to 2/2 Assembly-Worker artifact creature plus a {1},{T}
/// target-attacking-Assembly-Worker +2/+2 pump.
///
/// Mirrors <see cref="MishrasFactoryTests"/> (the near-identical-shape
/// analogue): Mishra's Foundry differs only in the animate cost ({2} vs
/// {1}), the pump cost ({1},{T} vs {T}), and the pump magnitude (+2/+2 vs
/// +1/+1).
/// </summary>
public class MishrasFoundryTests
{
    private readonly Player _alice = new("Alice", 20);

    [Fact]
    public void MishrasFoundry_IsLand_NoSubtypes()
    {
        var land = MishrasFoundryFactory.Create(_alice);
        ...
        land.Name.Should().Be("Mishra's Foundry");
    }

    [Fact]
    public void NamedCardFactory_Dispatches_MishrasFoundry()
    {
        var card = NamedCardFactory.Create("Mishra's Foundry", _alice);

        card.Should().BeOfType<Land>();
        card.Name.Should().Be("Mishra's Foundry");
    }

    [Fact]
    public void MishrasFoundry_HasTwoActivatedAbilities_AlongsideManaAbility()
    {
        var land = MishrasFoundryFactory.Create(_alice);
        ...
        // index 0 = {2} animate (no targets)
        activated[0].TargetRequests.Should().BeEmpty();

        // index 1 = {1},{T} pump (one target)
    ...
    public void Animate_RegistersLayer4AndLayer7b_WithAssemblyWorkerArtifactCreatureBody()
    {
        ...
        var land = MishrasFoundryFactory.Create(_alice, effects);
    ...
    [Fact]
    public void Pump_RegistersPlusTwoPlusTwoUntilEot_OnTargetAssemblyWorker()
    {
        var land = MishrasFoundryFactory.Create(_alice, effects: null);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        var pump = land.Abilities
            .Where(a => a.GetType() == typeof(ActivatedAbility))
            .Cast<ActivatedAbility>()
            .ElementAt(1); // {1},{T} pump

        // Pay the costs ({1} and tap).
    ...
    public void Pump_NonAssemblyWorkerTarget_IsNoOp()
    {
        ...
        // target. Mishra's Foundry's pump is restricted by printed
        // subtype.
        var land = MishrasFoundryFactory.Create(_alice, effects: null);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
