...
/// <summary>
/// Tests for Reforge the Soul (Avacyn Restored, {3}{R}{R}, Sorcery).
///
/// Oracle text:
...
///
/// Covers:
///   - Card identity (Sorcery, {3}{R}{R}, MV 5, owner/controller).
///   - NamedCardFactory dispatch.
///   - Miracle keyword marker (CR 702.94 — primitive deferred, marker
///     surfaces it for future wiring, same posture as Bonfire of the Damned).
///   - Resolve: both players' hands → graveyards, both players draw 7
///     (wheel effect — identical to Wheel of Fortune's body).
...
/// </summary>
public class ReforgeTheSoulTests
{
    ...
    [Fact]
    public void ReforgeTheSoul_IsSorcery_At3RR()
    {
        var card = ReforgeTheSoulFactory.Create(_alice);

        card.Name.Should().Be("Reforge the Soul");
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_ReforgeTheSoul()
    {
        var card = NamedCardFactory.Create("Reforge the Soul", _alice);
    ...
    public void Create_AttachesMiracleKeywordMarker()
    {
        ...
        card.Abilities.OfType<KeywordAbility>()
            ...
            .Should().Contain("Miracle",
                "Miracle primitive is deferred (CR 702.94) — marker surfaces the keyword for future wiring");
    }

    // -----------------------------------------------------------------------
    // Resolution — wheel: discard hand, draw 7
    // -----------------------------------------------------------------------
    ...
    public void Resolve_BothPlayersDiscardHand_ThenDrawSeven()
    {
        ...
        var effects = ReforgeTheSoulFactory.BuildResolveEffect(
            new[] { _alice, _bob });
