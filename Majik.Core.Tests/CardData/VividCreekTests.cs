...
/// <summary>
/// Unit tests for <see cref="VividCreekFactory"/> (Lorwyn "Vivid" land cycle).
///
/// Vivid Creek — Land.
...
///    color."
///
/// The exact cycle-mate of <see cref="VividMarshFactory"/>; only the base
/// colour differs ({B} -> {U}).
///
/// Covers:
/// - Identity (Land, nonbasic) + <see cref="NamedCardFactory"/> dispatch.
/// - ETB trigger places two charge counters (CR 122 / CR 614.1d).
/// - Six mana abilities: one base {U} (no counter cost) + five WUBRG
///   any-colour (each removes a charge counter).
/// - The base {U} ability activates with NO charge counters and removes none.
...
///   remain; the base {U} ability still is (it has no counter cost).
/// </summary>
public class VividCreekTests
{
    private readonly Player _alice = new("Alice", 20);

    /// <summary>
    /// The base "{T}: Add {U}" ability — the {U} producer with NO charge-counter
    /// cost. Identified behaviourally: with zero charge counters on an untapped
    /// land it is still activatable, whereas the any-colour {U} ability is not.
    /// </summary>
    private static ManaAbility BaseBlue(Land land)
    {
        EnsureZeroCountersOnBattlefield(land);
        return land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.Blue == 1 && m.CanActivate());
    }

    /// <summary>
    /// The any-colour {U} ability — the {U} producer that removes a charge
    /// counter. Identified behaviourally: with zero charge counters it is the
    /// blue ability that CANNOT activate (its remove-a-charge-counter cost is
    /// unpayable), whereas the base {U} ability can.
    /// </summary>
    private static ManaAbility AnyColorBlue(Land land)
    {
        EnsureZeroCountersOnBattlefield(land);
        return land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.Blue == 1 && !m.CanActivate());
    ...
    /// two {U} abilities when the land is untapped, on the battlefield, with no
    ...
    [Fact]
    public void VividCreek_Identity()
    {
        var land = VividCreekFactory.Create(_alice);

        land.Name.Should().Be("Vivid Creek");
        land.HasType(CardType.Land).Should().BeTrue();
        land.HasSupertype(CardSupertype.Basic).Should().BeFalse(
            "Vivid Creek is a nonbasic land");
        land.Owner.Should().BeSameAs(_alice);
        land.Controller.Should().BeSameAs(_alice);
    }

    [Fact]
    public void NamedCardFactory_Dispatches_VividCreek()
    {
        var card = NamedCardFactory.Create("Vivid Creek", _alice);

        card.Should().BeOfType<Land>();
        card!.Name.Should().Be("Vivid Creek");
        ...
        card.Abilities.OfType<ManaAbility>().Should().HaveCount(6,
            "one base {U} ability plus one any-colour ability per WUBRG");
    }
    ...
    [Fact]
    public void VividCreek_HasExactlyOneEtbTrigger()
    {
        var land = VividCreekFactory.Create(_alice);
    ...
    [Fact]
    public void VividCreek_Etb_PlacesTwoChargeCounters()
    {
        var land = VividCreekFactory.Create(_alice);
    ...
    [Fact]
    public void VividCreek_HasSixManaAbilities_BasePlusFiveAnyColor()
    {
        var land = VividCreekFactory.Create(_alice);
        var mas = land.Abilities.OfType<ManaAbility>().ToList();

        mas.Should().HaveCount(6, "one base {U} ability + five WUBRG any-colour abilities");

        // All five colours are produced (the any-colour suite), and {U} appears
        // twice (the base ability plus the any-colour blue slot).
        ...
        mas.Count(m => m.ManaGenerated.Blue == 1 && m.ManaGenerated.TotalValue == 1).Should().Be(2,
            "{U} is produced by both the base ability and the any-colour blue slot");

        // Behavioural split: on the battlefield with no charge counters, exactly
        // one ability (the base {U}) is activatable; the other five (the
        // charge-gated any-colour suite) are not.
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        mas.Count(m => m.CanActivate()).Should().Be(1,
            "with no charge counters only the base {U} ability (no counter cost) is activatable");
    ...
    [Fact]
    public void VividCreek_BaseBlue_ActivatesWithoutCounters_RemovesNone()
    {
        var land = VividCreekFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        var baseBlue = BaseBlue(land);
        baseBlue.CanActivate().Should().BeTrue(
            "the base {T}: Add {U} ability needs no charge counter");

        var produced = baseBlue.Activate();
        produced.Blue.Should().Be(1);
        produced.TotalValue.Should().Be(1);

        land.Counters.Count(CounterType.Charge).Should().Be(0,
            "the base {U} ability removes no charge counter");
    ...
    [Fact]
    public void VividCreek_AnyColor_Activate_RemovesChargeCounter_ProducesColor_AndTaps()
    {
        var land = VividCreekFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
    ...
    [Fact]
    public void VividCreek_CannotActivate_WhenTapped()
    {
        var land = VividCreekFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        // Resolve the base {U} ability before adding counters (the helper
    ...
    [Fact]
    public void VividCreek_NoChargeCounters_OnlyBaseBlueActivatable()
    {
        var land = VividCreekFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        AnyColorBlue(land).CanActivate().Should().BeFalse(
            "no charge counter to remove → the any-colour blue cost cannot be paid");

        // Exactly one mana ability — the base {U} — is activatable; the five
        ...
        land.Abilities.OfType<ManaAbility>().Count(m => m.CanActivate()).Should().Be(1,
            "only the base {T}: Add {U} ability (no charge-counter cost) is activatable");

        BaseBlue(land).CanActivate().Should().BeTrue(
            "the base {T}: Add {U} ability has no charge-counter cost");
    }
}
