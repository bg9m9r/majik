...
public class VividGroveTests
{
    ...
    [Fact]
    public void NamedCardFactory_Dispatches_VividGrove()
    {
        var card = NamedCardFactory.Create("Vivid Grove", _alice);

        card.Should().BeOfType<Land>();
        card!.Name.Should().Be("Vivid Grove");
        ...
        card.Abilities.OfType<ManaAbility>().Should().HaveCount(6,
            "one base {G} ability plus one any-colour ability per WUBRG");
    }
    ...
    public void VividGrove_AnyColor_Activate_RemovesChargeCounter_ProducesColor_AndTaps()
    {
        ...
        var anyWhite = land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.White == 1);

        anyWhite.CanActivate().Should().BeTrue(
            "untapped land with charge counters can pay {T} + remove a charge counter");

        var produced = anyWhite.Activate();
