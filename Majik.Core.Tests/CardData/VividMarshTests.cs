...
/// <summary>
/// Unit tests for <see cref="VividMarshFactory"/> (Lorwyn "Vivid" land cycle).
///
/// Vivid Marsh — Land.
...
///    color."
///
/// The exact cycle-mate of <see cref="VividCragFactory"/>; only the base
/// colour differs ({R} -> {B}).
///
/// Covers:
/// - Identity (Land, nonbasic) + <see cref="NamedCardFactory"/> dispatch.
/// - ETB trigger places two charge counters (CR 122 / CR 614.1d).
/// - Six mana abilities: one base {B} (no counter cost) + five WUBRG
///   any-colour (each removes a charge counter).
/// - The base {B} ability activates with NO charge counters and removes none.
...
///   remain; the base {B} ability still is (it has no counter cost).
/// </summary>
public class VividMarshTests
{
    private readonly Player _alice = new("Alice", 20);

    /// <summary>
    /// The base "{T}: Add {B}" ability — the {B} producer with NO charge-counter
    /// cost. Identified behaviourally: with zero charge counters on an untapped
    /// land it is still activatable, whereas the any-colour {B} ability is not.
    /// </summary>
    private static ManaAbility BaseBlack(Land land)
    {
        EnsureZeroCountersOnBattlefield(land);
        return land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.Black == 1 && m.CanActivate());
    }

    /// <summary>
    /// The any-colour {B} ability — the {B} producer that removes a charge
    /// counter. Identified behaviourally: with zero charge counters it is the
    /// black ability that CANNOT activate (its remove-a-charge-counter cost is
    /// unpayable), whereas the base {B} ability can.
    /// </summary>
    private static ManaAbility AnyColorBlack(Land land)
    {
        EnsureZeroCountersOnBattlefield(land);
        return land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.Black == 1 && !m.CanActivate());
    ...
    /// two {B} abilities when the land is untapped, on the battlefield, with no
    ...
    [Fact]
    public void VividMarsh_Identity()
    {
        var land = VividMarshFactory.Create(_alice);

        land.Name.Should().Be("Vivid Marsh");
        land.HasType(CardType.Land).Should().BeTrue();
        land.HasSupertype(CardSupertype.Basic).Should().BeFalse(
            "Vivid Marsh is a nonbasic land");
        land.Owner.Should().BeSameAs(_alice);
        land.Controller.Should().BeSameAs(_alice);
    }

    [Fact]
    public void NamedCardFactory_Dispatches_VividMarsh()
    {
        var card = NamedCardFactory.Create("Vivid Marsh", _alice);

        card.Should().BeOfType<Land>();
        card!.Name.Should().Be("Vivid Marsh");
        ...
        card.Abilities.OfType<ManaAbility>().Should().HaveCount(6,
            "one base {B} ability plus one any-colour ability per WUBRG");
    }
    ...
    [Fact]
    public void VividMarsh_HasExactlyOneEtbTrigger()
    {
        var land = VividMarshFactory.Create(_alice);
    ...
    [Fact]
    public void VividMarsh_Etb_PlacesTwoChargeCounters()
    {
        var land = VividMarshFactory.Create(_alice);
    ...
    [Fact]
    public void VividMarsh_HasSixManaAbilities_BasePlusFiveAnyColor()
    {
        var land = VividMarshFactory.Create(_alice);
        var mas = land.Abilities.OfType<ManaAbility>().ToList();

        mas.Should().HaveCount(6, "one base {B} ability + five WUBRG any-colour abilities");

        // All five colours are produced (the any-colour suite), and {B} appears
        // twice (the base ability plus the any-colour black slot).
        ...
        mas.Count(m => m.ManaGenerated.Black == 1 && m.ManaGenerated.TotalValue == 1).Should().Be(2,
            "{B} is produced by both the base ability and the any-colour black slot");

        // Behavioural split: on the battlefield with no charge counters, exactly
        // one ability (the base {B}) is activatable; the other five (the
        // charge-gated any-colour suite) are not.
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        mas.Count(m => m.CanActivate()).Should().Be(1,
            "with no charge counters only the base {B} ability (no counter cost) is activatable");
    ...
    [Fact]
    public void VividMarsh_BaseBlack_ActivatesWithoutCounters_RemovesNone()
    {
        var land = VividMarshFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        var baseBlack = BaseBlack(land);
        baseBlack.CanActivate().Should().BeTrue(
            "the base {T}: Add {B} ability needs no charge counter");

        var produced = baseBlack.Activate();
        produced.Black.Should().Be(1);
        produced.TotalValue.Should().Be(1);

        land.Counters.Count(CounterType.Charge).Should().Be(0,
            "the base {B} ability removes no charge counter");
    ...
    [Fact]
    public void VividMarsh_AnyColor_Activate_RemovesChargeCounter_ProducesColor_AndTaps()
    {
        var land = VividMarshFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
    ...
    [Fact]
    public void VividMarsh_CannotActivate_WhenTapped()
    {
        var land = VividMarshFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        // Resolve the base {B} ability before adding counters (the helper
    ...
    [Fact]
    public void VividMarsh_NoChargeCounters_OnlyBaseBlackActivatable()
    {
        var land = VividMarshFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        AnyColorBlack(land).CanActivate().Should().BeFalse(
            "no charge counter to remove → the any-colour black cost cannot be paid");

        // Exactly one mana ability — the base {B} — is activatable; the five
        ...
        land.Abilities.OfType<ManaAbility>().Count(m => m.CanActivate()).Should().Be(1,
            "only the base {T}: Add {B} ability (no charge-counter cost) is activatable");

        BaseBlack(land).CanActivate().Should().BeTrue(
            "the base {T}: Add {B} ability has no charge-counter cost");
    }
}
