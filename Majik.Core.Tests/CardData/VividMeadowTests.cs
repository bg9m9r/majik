...
/// <summary>
/// Unit tests for <see cref="VividMeadowFactory"/> (Lorwyn "Vivid" land cycle).
///
/// Vivid Meadow — Land.
...
///    color."
///
/// Exact cycle-mate of <see cref="VividCragFactory"/> with base colour {W}.
///
/// Covers:
/// - Identity (Land, nonbasic) + <see cref="NamedCardFactory"/> dispatch.
/// - ETB trigger places two charge counters (CR 122 / CR 614.1d).
/// - Six mana abilities: one base {W} (no counter cost) + five WUBRG
///   any-colour (each removes a charge counter).
/// - The base {W} ability activates with NO charge counters and removes none.
...
///   remain; the base {W} ability still is (it has no counter cost).
/// </summary>
public class VividMeadowTests
{
    private readonly Player _alice = new("Alice", 20);

    /// <summary>
    /// The base "{T}: Add {W}" ability — the {W} producer with NO charge-counter
    /// cost. Identified behaviourally: with zero charge counters on an untapped
    /// land it is still activatable, whereas the any-colour {W} ability is not.
    /// </summary>
    private static ManaAbility BaseWhite(Land land)
    {
        EnsureZeroCountersOnBattlefield(land);
        return land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.White == 1 && m.CanActivate());
    }

    /// <summary>
    /// The any-colour {W} ability — the {W} producer that removes a charge
    /// counter. Identified behaviourally: with zero charge counters it is the
    /// white ability that CANNOT activate (its remove-a-charge-counter cost is
    /// unpayable), whereas the base {W} ability can.
    /// </summary>
    private static ManaAbility AnyColorWhite(Land land)
    {
        EnsureZeroCountersOnBattlefield(land);
        return land.Abilities.OfType<ManaAbility>()
            .Single(m => m.ManaGenerated.White == 1 && !m.CanActivate());
    ...
    /// two {W} abilities when the land is untapped, on the battlefield, with no
    ...
    [Fact]
    public void VividMeadow_Identity()
    {
        var land = VividMeadowFactory.Create(_alice);

        land.Name.Should().Be("Vivid Meadow");
        land.HasType(CardType.Land).Should().BeTrue();
        land.HasSupertype(CardSupertype.Basic).Should().BeFalse(
            "Vivid Meadow is a nonbasic land");
        land.Owner.Should().BeSameAs(_alice);
        land.Controller.Should().BeSameAs(_alice);
    }

    [Fact]
    public void NamedCardFactory_Dispatches_VividMeadow()
    {
        var card = NamedCardFactory.Create("Vivid Meadow", _alice);

        card.Should().BeOfType<Land>();
        card!.Name.Should().Be("Vivid Meadow");
        ...
        card.Abilities.OfType<ManaAbility>().Should().HaveCount(6,
            "one base {W} ability plus one any-colour ability per WUBRG");
    }
    ...
    [Fact]
    public void VividMeadow_HasExactlyOneEtbTrigger()
    {
        var land = VividMeadowFactory.Create(_alice);
    ...
    [Fact]
    public void VividMeadow_Etb_PlacesTwoChargeCounters()
    {
        var land = VividMeadowFactory.Create(_alice);
    ...
    [Fact]
    public void VividMeadow_HasSixManaAbilities_BasePlusFiveAnyColor()
    {
        var land = VividMeadowFactory.Create(_alice);
        var mas = land.Abilities.OfType<ManaAbility>().ToList();

        mas.Should().HaveCount(6, "one base {W} ability + five WUBRG any-colour abilities");

        // All five colours are produced (the any-colour suite), and {W} appears
        // twice (the base ability plus the any-colour white slot).
        ...
        mas.Count(m => m.ManaGenerated.White == 1 && m.ManaGenerated.TotalValue == 1).Should().Be(2,
            "{W} is produced by both the base ability and the any-colour white slot");

        // Behavioural split: on the battlefield with no charge counters, exactly
        // one ability (the base {W}) is activatable; the other five (the
        // charge-gated any-colour suite) are not.
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        mas.Count(m => m.CanActivate()).Should().Be(1,
            "with no charge counters only the base {W} ability (no counter cost) is activatable");
    ...
    [Fact]
    public void VividMeadow_BaseWhite_ActivatesWithoutCounters_RemovesNone()
    {
        var land = VividMeadowFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        var baseWhite = BaseWhite(land);
        baseWhite.CanActivate().Should().BeTrue(
            "the base {T}: Add {W} ability needs no charge counter");

        var produced = baseWhite.Activate();
        produced.White.Should().Be(1);
        produced.TotalValue.Should().Be(1);

        land.Counters.Count(CounterType.Charge).Should().Be(0,
            "the base {W} ability removes no charge counter");
    ...
    [Fact]
    public void VividMeadow_AnyColor_Activate_RemovesChargeCounter_ProducesColor_AndTaps()
    {
        var land = VividMeadowFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
    ...
    [Fact]
    public void VividMeadow_CannotActivate_WhenTapped()
    {
        var land = VividMeadowFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);

        // Resolve the base {W} ability before adding counters (the helper
    ...
    [Fact]
    public void VividMeadow_NoChargeCounters_OnlyBaseWhiteActivatable()
    {
        var land = VividMeadowFactory.Create(_alice);
        _alice.Zones.Battlefield.AddCard(land);
        land.SetZone(ZoneType.Battlefield);
        ...
        AnyColorWhite(land).CanActivate().Should().BeFalse(
            "no charge counter to remove → the any-colour white cost cannot be paid");

        // Exactly one mana ability — the base {W} — is activatable; the five
        ...
        land.Abilities.OfType<ManaAbility>().Count(m => m.CanActivate()).Should().Be(1,
            "only the base {T}: Add {W} ability (no charge-counter cost) is activatable");

        BaseWhite(land).CanActivate().Should().BeTrue(
            "the base {T}: Add {W} ability has no charge-counter cost");
    }
}
