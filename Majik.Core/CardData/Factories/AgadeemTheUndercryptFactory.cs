...
///   "{T}: Add {B}."
///
/// Front face — <see cref="AgadeemsAwakeningFactory"/> (Sorcery {X}{B}{B}{B}).
///
/// ## MDFC infra
///
/// See <see cref="AgadeemsAwakeningFactory"/>'s class doc for the
...
/// <c>"Agadeem, the Undercrypt"</c> and lands here. The card is constructed
/// with its <see cref="MdfcState"/> pre-flipped to the back face so the
/// face tracker reads as authoritative.
///
/// ## Card identity comes from JSON
///
/// Name / type and the <b>{T}: Add {B}</b> mana ability are loaded from the
/// embedded JSON definition (<c>agadeem-the-undercrypt.json</c>) via
...
///   (mirrors <see cref="ShatterskullTheHammerPassFactory"/>).
...
/// ## References
///
/// - <see cref="ShatterskullTheHammerPassFactory"/> — identical "pay 3 life
///   or enters tapped" + tap-for-one-mana back-face land; this factory
///   directly mirrors it (swapping {R} → {B}).
/// </summary>
[CardName("Agadeem, the Undercrypt")]
public static class AgadeemTheUndercryptFactory
{
    public const string CardName = "Agadeem, the Undercrypt";
    public const string FrontName = "Agadeem's Awakening";

    /// <summary>
    /// Construct Agadeem, the Undercrypt without a
    /// <see cref="ReplacementBus"/>. The ETB-tapped predicate is omitted;
    /// the {T}: Add {B} mana ability (from JSON) is still wired. Suitable
    /// for card-shape / dispatcher tests.
    /// </summary>
    public static Land Create(Player owner) => Create(owner, replacements: null);

    /// <summary>
    /// Construct Agadeem, the Undercrypt with an optional
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        var definition = CardDefinitionLoader.FromEmbeddedResource("agadeem-the-undercrypt");
        var land = (Land)CardDefinitionFactory.Build(definition, owner);

        // CR 711 / 712 — attach the MDFC face tracker pre-flipped to the
        // back face (Agadeem, the Undercrypt is the back face that actually
    ...
    private static bool TryPayThreeLifeOrEnterTapped(Player controller, ICard self)
    {
        ...
        if (agent == null)
        {
            ...
            // matching ShatterskullTheHammerPassFactory).
            return false;
        }
        ...
        try
        {
            wantsToPay = agent.ChooseYesNoAsync(
                question: "Pay 3 life so Agadeem, the Undercrypt enters untapped?",
