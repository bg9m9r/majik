...
///   "{T}: Add {R}."
///
/// Front face — <see cref="AkoumWarriorFactory"/> (Creature — Minotaur
/// Warrior {5}{R} 4/5 with Trample).
///
/// ## MDFC infra
///
/// See <see cref="AkoumWarriorFactory"/>'s class doc for the cast-either-face
...
/// back-face name <c>"Akoum Teeth"</c> and lands here. The card is
...
/// embedded JSON definition (<c>akoum-teeth.json</c>) via
...
///   <see cref="KazanduValleyFactory"/>'s back-face posture).
/// - <b>{T}: Add {R}</b> — single <see cref="ManaAbility"/> producing one red
...
///   <see cref="ReplacementBus"/>. Akoum Teeth ALWAYS enters tapped — no
...
///   <see cref="KazanduValleyFactory"/>'s no-bus overload).
///
/// ## References
///
/// - <see cref="KazanduValleyFactory"/> — sibling ZNR MDFC back-face tapland
///   that taps for a single color; this factory mirrors its MDFC /
///   mana-ability / unconditional enters-tapped wiring (swapping green for
///   red).
/// </summary>
[CardName("Akoum Teeth")]
public static class AkoumTeethFactory
{
    public const string CardName = "Akoum Teeth";
    public const string FrontName = "Akoum Warrior";
    public const string Slug = "akoum-teeth";

    /// <summary>
    /// Construct Akoum Teeth without a <see cref="ReplacementBus"/>. The ETB
    /// "enters tapped" replacement is omitted; the {T}: Add {R} mana ability
    ...
    /// <summary>
    /// Construct Akoum Teeth with an optional <see cref="ReplacementBus"/>
    /// for full ETB wiring.
    /// </summary>
