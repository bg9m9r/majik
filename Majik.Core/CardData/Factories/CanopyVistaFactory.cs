...
/// <summary>
/// Named-card factory for Canopy Vista (Battle for Zendikar) — a member of
/// the "battle land" / "tango land" nonbasic dual cycle. Oracle text:
///   "({T}: Add {G} or {W}.)
///    This land enters tapped unless you control two or more basic lands."
///
/// <para>
/// The Land shell — both printed land subtypes (Forest / Plains) plus the
/// two mana abilities {G}/{W} (CR 605.1 — mana abilities don't use the
/// stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/canopy-vista.json</c> and materialized via
...
/// </para>
/// </summary>
[CardName("Canopy Vista")]
public static class CanopyVistaFactory
{
    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("canopy-vista");

    /// <summary>Construct Canopy Vista owned and controlled by
    /// <paramref name="owner"/> (shape-only path — no ETB-tapped
    /// replacement wired).</summary>
    public static Land Create(Player owner) => Create(owner, replacements: null);

    /// <summary>Construct Canopy Vista with an optional
