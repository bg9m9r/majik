...
/// <summary>
/// Named-card factory for Cinder Glade (Battle for Zendikar) — a member of
/// the "battle land" / "tango land" nonbasic dual cycle. Oracle text:
///   "({T}: Add {R} or {G}.)
///    This land enters tapped unless you control two or more basic lands."
///
/// <para>
/// The Land shell — both printed land subtypes (Mountain / Forest) plus the
/// two mana abilities {R}/{G} (CR 605.1 — mana abilities don't use the
/// stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/cinder-glade.json</c> and materialized via
/// <see cref="CardDefinitionFactory"/>, the same posture as
/// <see cref="ZagothTriomeFactory"/>. Mana flows from the printed subtypes
...
///
/// <para>
/// This is the same shape as <see cref="CheckLandCycleFactory"/> — only the
/// predicate differs: check lands ask "do you control a [BasicA] or [BasicB]"
/// (subtype match), battle lands ask "do you control two or more basic lands"
/// (Basic-supertype count). The dedicated
/// <see cref="ConditionalEntersTappedBinder"/> only claims the
/// "N or more other lands" oracle form, so this count variant is wired here
/// in the factory rather than in a generic binder.
/// </para>
///
/// <para>
...
public static class CinderGladeFactory
{
    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("cinder-glade");

    /// <summary>Construct Cinder Glade owned and controlled by
    /// <paramref name="owner"/> (shape-only path — no ETB-tapped
    /// replacement wired).</summary>
    public static Land Create(Player owner) => Create(owner, replacements: null);

    /// <summary>Construct Cinder Glade with an optional
