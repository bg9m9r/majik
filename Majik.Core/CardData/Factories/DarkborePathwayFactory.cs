...
///   "{T}: Add {B}."
///
/// Back face — <see cref="SlitherborePathwayFactory"/> (Land —
/// "{T}: Add {G}.").
///
...
///     <c>"Darkbore Pathway"</c> → this factory → a <see cref="Land"/>
///     with <c>{T}: Add {B}</c>.</item>
///   <item>Playing the back face → <see cref="NamedCardFactory"/> resolves
///     <c>"Slitherbore Pathway"</c> → <see cref="SlitherborePathwayFactory"/>
...
/// embedded JSON definition (<c>darkbore-pathway.json</c>) via
...
/// JSON-driven shape as <see cref="BlightstepPathwayFactory"/>.
///
/// ## Implemented (v1)
///
/// - Non-Basic <see cref="Land"/> with no printed subtype/supertype.
///   Owner / controller wired (from JSON).
/// - <see cref="MdfcState"/> attached (front = "Darkbore Pathway",
///   back = "Slitherbore Pathway"); starts on the front face.
...
///   which a pathway has).
/// </summary>
[CardName("Darkbore Pathway")]
public static class DarkborePathwayFactory
{
    public const string CardName = "Darkbore Pathway";
    public const string BackName = "Slitherbore Pathway";

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("darkbore-pathway");

    /// <summary>
    /// Construct the front face Darkbore Pathway owned and controlled by
