...
/// <summary>
/// Named-card factory for Deserted Beach (Innistrad: Midnight Hunt) — the
/// W/U member of the "slow land" cycle. Oracle (verified against Scryfall):
/// <code>
...
/// printed subtype) plus the two mana abilities {W}/{U} (CR 605.1 — mana
/// abilities don't use the stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/deserted-beach.json</c> and materialized via
/// <see cref="CardDefinitionFactory"/>, the same JSON-driven posture as
/// <see cref="SeachromeCoastFactory"/> (its fast-land counterpart).
/// </para>
///
/// <para>
/// "Enters tapped unless you control two or MORE other lands" (CR 614.1c) is
/// the slow-land ETB condition — the inverse of the fast land's "two or fewer".
...
/// the predicate <c>CountOtherLands(controller, self) &gt;= 2</c> on the
/// "more" branch). This factory mirrors that predicate on the optional
/// <see cref="ReplacementBus"/> overload so the behaviour is exercisable in
/// isolation (same shape as <see cref="SeachromeCoastFactory"/>): the land
/// enters untapped iff the controller controls at least two OTHER lands
/// ("other" excludes this slow land via reference equality; "you control"
...
public static class DesertedBeachFactory
{
    ...
    /// <summary>
    /// Construct Deserted Beach owned and controlled by
    ...
    /// <summary>
    /// Construct Deserted Beach with an optional <see cref="ReplacementBus"/>
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        // on the production load path ("more" branch ⇒ >= threshold).
        // ----------------------------------------------------------------
