...
/// battlefield at resolution, the effect does nothing. The nonland
/// restriction is also enforced at resolution — a land passed as a raw
/// target yields a no-op.
/// </summary>
[CardName("Disperse")]
public static class DisperseFactory
{
    ...
    public static SpellDefinition BuildDefinition(ZoneService? zoneService = null) =>
        new(
            ...
            EffectFactory: p =>
            {
                ...
                {
                    new Effect(
                        "Disperse — return target nonland permanent to its owner's hand",
    ...
    private static void Resolve(object raw, ZoneService? zoneService)
    {
        ...
        // Disperse restricts to nonland permanents — no-op if target is a land.
