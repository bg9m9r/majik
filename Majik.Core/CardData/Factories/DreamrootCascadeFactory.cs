...
/// <summary>
/// Named-card factory for Dreamroot Cascade (Wilds of Eldraine) — the G/U
...
/// printed subtype) plus the two mana abilities {G}/{U} (CR 605.1 — mana
/// abilities don't use the stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/dreamroot-cascade.json</c> and materialized
...
/// FEWER" threshold. On the production load path it is wired automatically
/// from the printed oracle text by <see cref="ConditionalEntersTappedBinder"/>
/// (its "N or [more|fewer] other lands" regex matches this exact wording,
/// building the predicate <c>CountOtherLands(controller, self) &gt;= 2</c>).
...
public static class DreamrootCascadeFactory
{
    ...
    /// <summary>
    /// Construct Dreamroot Cascade owned and controlled by
    ...
    /// <summary>
    /// Construct Dreamroot Cascade with an optional <see cref="ReplacementBus"/>
