...
///   "{T}: Add {W}."
///
/// Front face — <see cref="EmeriasCallFactory"/> (Sorcery {4}{W}{W}{W}).
///
/// ## MDFC infra
///
/// See <see cref="EmeriasCallFactory"/>'s class doc for the cast-either-face
...
/// back-face name <c>"Emeria, Shattered Skyclave"</c> and lands here. The
/// card is constructed with its <see cref="MdfcState"/> pre-flipped to the
/// back face so the face tracker reads as authoritative.
///
/// ## Card identity comes from JSON
///
/// Name / type and the <b>{T}: Add {W}</b> mana ability are loaded from the
/// embedded JSON definition (<c>emeria-shattered-skyclave.json</c>) via
...
/// - <b>ETB "you may pay 3 life; if you don't, it enters tapped" (CR 614.1c)</b>
///   — modelled via <see cref="ConditionalEntersTappedReplacement"/> on the
///   supplied <see cref="ReplacementBus"/>. Predicate mirrors
///   <see cref="AgadeemTheUndercryptFactory"/>:
///     <list type="bullet">
...
///   ETB replacement is omitted (shape-only posture).
///
/// ## References
///
/// - <see cref="AgadeemTheUndercryptFactory"/> — identical "pay 3 life or
///   enters tapped" + tap-for-one-mana ZNR MDFC back-face land; this factory
///   directly mirrors it (swapping {B} → {W}).
/// </summary>
[CardName("Emeria, Shattered Skyclave")]
public static class EmeriaShatteredSkyclaveFactory
{
    public const string CardName = "Emeria, Shattered Skyclave";
    public const string FrontName = "Emeria's Call";

    /// <summary>
    /// Construct Emeria, Shattered Skyclave without a
    /// <see cref="ReplacementBus"/>. The ETB-tapped predicate is omitted; the
    /// {T}: Add {W} mana ability (from JSON) is still wired. Suitable for
    /// card-shape / dispatcher tests.
    /// </summary>
    public static Land Create(Player owner) => Create(owner, replacements: null);

    /// <summary>
    /// Construct Emeria, Shattered Skyclave with an optional
    /// <see cref="ReplacementBus"/> for full ETB wiring.
    /// </summary>
    /// <param name="owner">Card owner / initial controller.</param>
    /// <param name="replacements">When supplied, the "you may pay 3 life; if
    /// you don't, it enters tapped" replacement is registered
    /// (CR 614.1c).</param>
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        var definition = CardDefinitionLoader.FromEmbeddedResource("emeria-shattered-skyclave");
        var land = (Land)CardDefinitionFactory.Build(definition, owner);

        // CR 711 / 712 — attach the MDFC face tracker pre-flipped to the back
        // face (Emeria, Shattered Skyclave is the back face that actually
    ...
    /// <summary>
    /// Predicate body for the ETB replacement. Honours CR 119.4 ("you can't
    /// pay life you don't have"), consults the controller's registered agent
    /// for the optional payment, and deducts 3 life on the yes path.
    /// </summary>
    ...
    private static bool TryPayThreeLifeOrEnterTapped(Player controller, ICard self)
    {
        ...
        // CR 119.4 — a payment bringing the total to 0 is legal; below 3 the
        ...
        try
        {
            wantsToPay = agent.ChooseYesNoAsync(
                question: "Pay 3 life so Emeria, Shattered Skyclave enters untapped?",
