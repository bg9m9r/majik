...
///
/// WU Verge cycle — counterpart to Gloomlake Verge (UB), Bleachbone Verge (WB),
/// Wastewood Verge (GB), Sunsplit Verge (RW), and Gleamfield Verge (GW).
///
...
/// control a Plains or an Island at activation time, the {U} ability is illegal
...
/// rule needed.
///
/// "Control a Plains or an Island" (CR 305.6 / 205.3i) — any permanent the
/// controller controls on the battlefield whose subtype list includes Plains
/// OR Island satisfies the restriction. Basic Plains/Island, nonbasic
/// Plains/Island-typed dual lands (e.g. Hallowed Fountain), and type-granting
/// effects all qualify.
///
/// ## Implementation
///
/// Mirrors <see cref="BleachboneVergeFactory"/> / <see cref="GloomlakeVergeFactory"/> —
/// the JSON/CardDefinitionFactory path cannot express the "activate only if you
/// control a subtype" mana-ability restriction (ManaAbilityDefinition exposes
/// only the produced mana), so the land is built directly here using the
/// ManaAbility canActivateCheck seam. The companion JSON
/// (CardData/Cards/floodfarm-verge.json) documents the produced-mana shape for
/// cycle parity.
/// </summary>
...
public static class FloodfarmVergeFactory
{
    ...
    public static Land Create(Player owner)
    {
        ...
        var land = new Land("Floodfarm Verge");
        land.SetOwner(owner);
        ...
        // battlefield has the Plains or Island subtype (CR 305.6 / 205.3i).
        ...
        land.AddAbility(new ManaAbility(
            ...
            canActivateCheck: () =>
            {
                ...
                // at least one permanent with subtype Plains or Island.
                // CR 305.6 / 205.3i — any permanent (land, creature-land, etc.)
                // with the Plains or Island subtype qualifies.
