...
/// <summary>
/// Named-card factory for Furycalm Snarl (Strixhaven: School of Mages) — a
/// member of the "Snarl" reveal-land cycle (Frostboil Snarl, Necroblossom
...
/// <code>
/// As this land enters, you may reveal a Mountain or Plains card from your hand.
/// If you don't, this land enters tapped.
/// {T}: Add {R} or {W}.
/// </code>
///
/// The type line is a bare <c>Land</c> — no printed land subtype — so the two
/// mana modes (<c>{T}: Add {R} or {W}</c>) are wired explicitly as two painless
...
/// hand subtypes (Mountain / Plains) and the produced colours ({R} / {W})
/// differ.
///
/// ## Implemented (v1)
/// - <b>Identity + {R}/{W} mana</b> — loaded from
///   <c>Majik.Core/CardData/Cards/furycalm-snarl.json</c> via
///   <see cref="CardDefinitionFactory"/>: a nonbasic <see cref="Land"/> with no
///   printed subtype and two painless mana abilities producing {R} and {W}.
/// - <b>ETB "you may reveal a Mountain or Plains; if you don't, it enters
...
///   hand match is a card carrying the <see cref="CardSubtype.Mountain"/> <i>or</i>
///   <see cref="CardSubtype.Plains"/> subtype (CR 205.4a — a card "is a
///   Mountain" / "is a Plains" by subtype):
...
///   the bus is supplied.
/// </summary>
[CardName("Furycalm Snarl")]
public static class FurycalmSnarlFactory
{
    public const string CardName = "Furycalm Snarl";

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("furycalm-snarl");

    /// <summary>
    /// Construct Furycalm Snarl with no ETB wiring (single-arg dispatcher
    /// path). The reveal-or-tapped replacement is omitted — no
    /// <see cref="ReplacementBus"/> available — matching the reveal / shock /
    /// check land cycle shape-only posture. The {R}/{W} mana modes are still
    /// attached.
    /// </summary>
    public static Land Create(Player owner) => Create(owner, replacements: null);

    /// <summary>
    /// Construct Furycalm Snarl. When <paramref name="replacements"/> is
    /// supplied the ETB "you may reveal a Mountain or Plains; if you don't, it
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        // Identity (bare Land, no subtype) + the two painless {T}: Add {R}/{W}
        // mana abilities come from the JSON definition.
        var land = (Land)CardDefinitionFactory.Build(Definition, owner);

        // ----------------------------------------------------------------
        // ETB: "As this land enters, you may reveal a Mountain or Plains card
        ...
        // returns true (untapped) iff the controller has a Mountain/Plains
        ...
        if (replacements != null)
        {
            replacements.Register(new ConditionalEntersTappedReplacement(
                land,
                entersUntappedIf: (controller, _) => TryRevealMountainOrPlains(controller)));
        }
    ...
    /// Plains card in hand first (CR 205.4a — match by the Mountain/Plains
    /// subtype), then consults the controller's registered agent for the
    /// optional reveal.
    /// </summary>
    /// <returns><c>true</c> ⇒ land enters untapped (a Mountain/Plains was
    /// revealed). <c>false</c> ⇒ enters tapped (no matching card in hand,
    /// declined, or no agent registered).</returns>
    private static bool TryRevealMountainOrPlains(Player controller)
    {
        ...
        var hasMatch = controller.Zones.Hand.GetCards()
            .Any(c => c.HasSubtype(CardSubtype.Mountain) || c.HasSubtype(CardSubtype.Plains));
        ...
        try
        {
            ...
            return agent.ChooseYesNoAsync(
                question: "Reveal a Mountain or Plains card so this land enters untapped?",
