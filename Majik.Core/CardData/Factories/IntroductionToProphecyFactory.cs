...
/// <summary>
/// Named-card factory for Introduction to Prophecy (Strixhaven: School of
/// Mages, {3}).
///
...
/// Scry before draw (CR 701.20 then CR 121.1). Functionally identical resolve
/// to Preordain / Deliberate at the colorless cost {3}. Agent scry decision
/// flows through <see cref="Majik.Core.Players.Agents.AgentRegistry"/>; an
...
///
/// ## Lesson subtype
/// The printed "Lesson" subtype is OMITTED — <see cref="CardSubtype"/> has no
/// Lesson member and Lesson carries no gameplay rules of its own (it only
/// gates the Learn keyword action, CR 702.149, which is not modelled), so its
/// absence is behaviourally lossless. Same posture as
/// <see cref="DaiLiIndoctrinationFactory"/> / <see cref="OriginOfMetalbendingFactory"/>.
/// </summary>
[CardName("Introduction to Prophecy")]
public static class IntroductionToProphecyFactory
{
    public const string CardName = "Introduction to Prophecy";
    ...
    /// <summary>
    /// Build Introduction to Prophecy's resolve effect — scry 2, then draw a
    /// card. Returns a SINGLE composite <see cref="IEffect"/> so the legacy
    /// <c>.Single()</c> caller contract holds.
    /// </summary>
