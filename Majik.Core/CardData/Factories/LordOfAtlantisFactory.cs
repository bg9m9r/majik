...
/// <summary>
/// Named-card factory for Lord of Atlantis (Limited Edition Alpha / many
/// reprints, Creature — Merfolk {U}{U} 2/2).
///
/// Oracle text:
...
/// - <b>Static "Other Merfolk get +1/+1 and have Islandwalk"</b> wired via
...
///   <c>allPlayers: true</c>.
///   The <c>allPlayers: true</c> flag means ALL Merfolk on the battlefield
///   are affected — including an opponent's Merfolk — which matches the
///   symmetric Oracle text ("Other Merfolk", not "Other Merfolk you
///   control"). Layer 7c for P/T (CR 613.7c), Layer 6 for the granted
...
public static class LordOfAtlantisFactory
{
    ...
    /// <summary>
    /// Construct Lord of Atlantis without a live continuous-effects service.
    /// Suitable for shape / dispatcher tests — the lord static effect is
    /// not registered. Other Merfolk don't yet receive +1/+1 + Islandwalk
    ...
    /// <summary>
    /// Construct a fully-wired Lord of Atlantis. When
    /// <paramref name="continuousEffects"/> is supplied, a
    /// <see cref="LordStaticEffect"/> granting +1/+1 and Islandwalk to
    /// all OTHER Merfolk on the battlefield (including opponents') is
    ...
    public static Creature Create(Player owner, ContinuousEffectsService? continuousEffects)
    {
        ...
        if (continuousEffects != null)
        {
            ...
            // "Other Merfolk get +1/+1 and have Islandwalk." — note no
            // "you control" qualifier, so allPlayers: true applies the
            // bonus symmetrically to ALL Merfolk on the battlefield,
            // including opponents'. includeSelf: false honours "Other".
