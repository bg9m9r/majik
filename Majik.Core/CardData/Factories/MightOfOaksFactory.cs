...
/// <summary>
/// Named-card factory for Might of Oaks (Urza's Legacy, {3}{G}).
///
/// Instant. Oracle text:
///   "Target creature gets +7/+7 until end of turn."
///
/// ## Implemented (v1)
/// - Instant card with printed mana cost {3}{G}.
/// - <see cref="BuildDefinition"/> wires the resolve effect:
///   1..1 "target creature" <see cref="TargetRequest"/>. On resolve:
///   register a <see cref="PumpUntilEndOfTurnEffect"/>(+7, +7) on the
...
///
/// Mirrors <see cref="GiantGrowthFactory"/> — same shape at +7/+7 for {3}{G}.
/// </summary>
[CardName("Might of Oaks")]
public static class MightOfOaksFactory
{
    public const string CardName = "Might of Oaks";
    public const string PrintedManaCost = "{3}{G}";

    /// <summary>Layer 7c +P/+T magnitude (CR 613.1g).</summary>
    public const int PumpAmount = 7;
    ...
    /// <summary>
    /// Build the "target creature gets +7/+7 until end of turn"
    ...
    /// valid, registers a <see cref="PumpUntilEndOfTurnEffect"/>(+7, +7)
    ...
    public static SpellDefinition BuildDefinition() =>
        new(
            ...
            EffectFactory: p =>
            {
                ...
                {
                    new Effect(
                        "Might of Oaks — target creature gets +7/+7 until end of turn",
                        () => Resolve(raw)),
                };
            });
