...
/// modal double-faced card <c>Ondu Inversion // Ondu Skyruins</c> ({6}{W}{W}).
///
...
/// ("Ondu Inversion // Ondu Skyruins"). The two individual faces are already
/// implemented by <see cref="OnduInversionFactory"/> (front) and
/// <see cref="OnduSkyruinsFactory"/> (back), but the engine's
...
/// combined name resolves to the FRONT face (Ondu Inversion, a Sorcery). The
...
/// <c>MdfcCastFlow</c> can offer "play as the land Ondu Skyruins" with its
/// unconditional "this land enters tapped" ETB — CR 614.1c). This arm simply
/// delegates to <see cref="OnduInversionFactory.Create"/> so the
/// cast-either-face wiring is defined in exactly one place.
/// </summary>
[CardName("Ondu Inversion // Ondu Skyruins")]
public static class OnduInversionOnduSkyruinsFactory
{
    public const string CardName = "Ondu Inversion // Ondu Skyruins";
    public const string Slug = "ondu-inversion-ondu-skyruins";

    /// <summary>
    /// Build the combined-name MDFC as its castable FRONT face (Ondu
    /// Inversion, Sorcery {6}{W}{W}) with the <see cref="MdfcState"/> face
    ...
    /// board-wipe behaviour come from <see cref="OnduInversionFactory"/> so the
    ...
    public static Sorcery Create(Player owner)
    {
        ...
        return OnduInversionFactory.Create(owner);
    }
}
