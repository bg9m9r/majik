...
/// <summary>
/// Named-card factory for Open Fire (many sets, {2}{R}).
///
/// Instant. Oracle text:
///   "Open Fire deals 3 damage to any target."
///
/// A {2}{R} variant of Lightning Strike — same 3-damage payload but at
/// one generic mana higher. Same resolve shape as
/// <see cref="LightningStrikeFactory"/> / <see cref="LightningBoltFactory"/>,
/// routed through <see cref="Fx.DealDamageAny"/> so creature / player /
/// planeswalker / battle targets all resolve correctly (CR 115.3).
/// </summary>
[CardName("Open Fire")]
public static class OpenFireFactory
{
    public const string CardName = "Open Fire";
    ...
    /// <summary>
    /// Build the <see cref="SpellDefinition"/> used when Open Fire
    ...
    public static SpellDefinition BuildSpellDefinition(Func<object, object> resolver)
    {
        ...
        return new SpellDefinition(
            ...
            EffectFactory: chosen =>
            {
                ...
                return new IEffect[]
                {
                    Fx.Inline("Open Fire: 3 damage to any target", () =>
