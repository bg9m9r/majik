...
/// <summary>
/// Named-card factory for Plummet (Magic 2012, {1}{G}).
///
/// Instant. Oracle text:
///   "Destroy target creature with flying."
///
/// ## Implemented (v1)
/// - Instant shape, mana cost {1}{G}, owner / controller.
/// - <b>Destroy target creature with flying</b> — <see cref="BuildDefinition"/>
///   builds a <see cref="SpellDefinition"/> with a single 1..1
///   "target creature with flying" <see cref="TargetRequest"/>. On resolution
///   the chosen creature is checked for the Flying keyword (CR 702.9 — Flying)
///   via <see cref="CombatAbilities.HasFlying"/> and destroyed via
///   <see cref="OracleSpellBinder.MoveToGraveyard"/>
///   (CR 701.7) iff it is still a Creature on the Battlefield with Flying
...
/// posture as <see cref="DoomBladeFactory"/> / <see cref="VictimOfNightFactory"/>.
/// </summary>
[CardName("Plummet")]
public static class PlummetFactory
{
    public const string CardName = "Plummet";
    public const string PrintedManaCost = "{1}{G}";

    /// <summary>CardDef DSL — card shape only. Resolve behaviour (destroy
    /// target creature with flying) is built on demand via
    ...
    /// <summary>
    /// Build the "destroy target creature with flying" <see cref="SpellDefinition"/>.
    ///
    /// On resolve: validates target is still a Creature on the Battlefield
    /// AND has the Flying keyword (CR 608.2b — illegal-target filter at
        ...
        return new SpellDefinition(
            ...
            TargetRequests: new[]
            {
                new TargetRequest(
                    ...
                    // Agent-prompt MVP: live gather every creature with Flying
                    // on any battlefield. Removal intent in the bot's ranker
                    // pushes the opponent's biggest flying threat up.
            ...
            EffectFactory: p =>
            {
                ...
                {
                    new Effect(
                        $"{CardName}: destroy target creature with flying",
                        () =>
                        {
                            // CR 608.2b — resolution-time legality re-check.
                            if (resolved is not Creature target) return;
                            if (target.Zone != ZoneType.Battlefield) return;
                            // CR 702.9 — Flying: target must still have Flying
                            // at resolution (e.g. could have lost it via
                            // LoseKeywordEffect).
