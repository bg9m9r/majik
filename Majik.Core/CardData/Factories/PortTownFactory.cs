...
/// <summary>
/// Named-card factory for Port Town (Shadows over Innistrad) — a member of the
/// SOI/EMN "slow land" / "buddy land" dual cycle. Oracle text:
///   "As this land enters, you may reveal a Plains or Island card from your
///    hand. If you don't, this land enters tapped.
///    {T}: Add {W} or {U}."
///
/// <para>
/// The Land shell — both mana abilities {W}/{U} (CR 605.1 — mana abilities
/// don't use the stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/port-town.json</c> and materialized via
/// <see cref="CardDefinitionFactory"/>, the same posture as
/// <see cref="CinderGladeFactory"/>. Port Town is a nonbasic, non-typed land
...
/// contains at least one Plains-or-Island card (i.e. there is a card available
/// to reveal). "Plains or Island card" is matched by land subtype
/// (<see cref="ICard.HasSubtype"/>) — CR 205.3i / 305.6: a card is a "Plains
/// card" iff it has the Plains land subtype — so dual-typed and nonbasic lands
/// carrying the Plains/Island subtype qualify, matching the printed oracle.
...
///
/// <para>
/// This mirrors <see cref="CinderGladeFactory"/>'s scaffold, swapping the
/// battle-land "two or more basic lands on the battlefield" predicate for the
/// buddy-land "a Plains-or-Island card in hand" predicate.
/// </para>
///
/// <para>
...
/// </summary>
[CardName("Port Town")]
public static class PortTownFactory
{
    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("port-town");

    /// <summary>Construct Port Town owned and controlled by
    /// <paramref name="owner"/> (shape-only path — no ETB-tapped
    /// replacement wired).</summary>
    public static Land Create(Player owner) => Create(owner, replacements: null);

    /// <summary>Construct Port Town with an optional
    /// <see cref="ReplacementBus"/> for full "you may reveal a Plains or Island
    /// card from your hand; if you don't, it enters tapped" wiring
    /// (CR 614.1c).</summary>
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        // ----------------------------------------------------------------
        // "You may reveal a Plains or Island card from your hand. If you
        // don't, this land enters tapped." (CR 614.1c). The reveal is a
        // cost-free, downside-free choice, so the engine auto-reveals: the
        // land enters untapped iff the controller has a Plains-or-Island
        // card to reveal in hand. Predicate returns true => untapped,
        // false => tapped. "Plains or Island card" => a card with the
        // Plains or Island land subtype (CR 205.3i); the entering land is
    ...
    private static bool ControllerHasRevealableCardInHand(Player controller, ICard self) =>
        controller.Zones.Hand.GetCards()
            .Any(c => !ReferenceEquals(c, self)
                && (c.HasSubtype(CardSubtype.Plains) || c.HasSubtype(CardSubtype.Island)));
}
