...
/// <summary>
/// Named-card factory for Scorching Shot (Champions of Kamigawa, {R}{R}).
///
/// Sorcery. Oracle text:
...
/// ## Implementation
///
/// Same shape as <see cref="FlameSlashFactory"/> (a Sorcery dealing fixed
/// damage to a single target creature) but priced at {R}{R} with a 5-damage
/// payload.
///
/// - <b>Sorcery</b> shape, mana cost {R}{R} (double red).
...
/// - CR 608.2b — if the resolved target is not a creature (e.g. it left the
///   battlefield after targeting), the effect is a no-op rather than
///   redirecting the damage elsewhere.
/// </summary>
[CardName("Scorching Shot")]
public static class ScorchingShotFactory
{
    public const string CardName = "Scorching Shot";
    public const string PrintedManaCost = "{R}{R}";
    public const int Damage = 5;

    /// <summary>CardDef DSL — card shape only (Sorcery, {R}{R}).
    ...
    /// <summary>
    /// Build the <see cref="SpellDefinition"/> used when Scorching Shot is
    /// cast. Single 1..1 "target creature" request; on resolution deals
    /// <see cref="Damage"/> (5) damage to the chosen creature through
    /// <see cref="Fx.DealDamageAny"/>.
    ///
    /// CR 608.2b — if the resolved object is not a creature (illegal target
    /// due to a zone change or type change after targeting), the effect is
    /// silently skipped.
    /// </summary>
    ...
    public static SpellDefinition BuildSpellDefinition(Func<object, object> resolver)
    {
        ...
        return new SpellDefinition(
            ...
            EffectFactory: chosen =>
            {
                ...
                return new IEffect[]
                {
                    Fx.Inline("Scorching Shot: 5 damage to target creature", () =>
                    {
