...
/// Sea Gate Restoration // Sea Gate, Reborn (Zendikar Rising).
///
...
///   "{T}: Add {U}."
///
/// Front face — <see cref="SeaGateRestorationFactory"/> (Sorcery {4}{U}{U}{U}).
///
/// ## MDFC infra
///
/// See <see cref="SeaGateRestorationFactory"/>'s class doc for the
...
/// <c>"Sea Gate, Reborn"</c> and lands here. The card is constructed with its
...
/// embedded JSON definition (<c>sea-gate-reborn.json</c>) via
...
///   blue mana (CR 605.1 — mana ability, no stack), from JSON.
...
///   enters tapped" + tap-for-one-mana back-face land; this factory directly
///   mirrors it (swapping {B} → {U}).
/// </summary>
[CardName("Sea Gate, Reborn")]
public static class SeaGateRebornFactory
{
    public const string CardName = "Sea Gate, Reborn";
    public const string FrontName = "Sea Gate Restoration";

    /// <summary>
    /// Construct Sea Gate, Reborn without a <see cref="ReplacementBus"/>.
    /// The ETB-tapped predicate is omitted; the {T}: Add {U} mana ability
    ...
    /// <summary>
    /// Construct Sea Gate, Reborn with an optional
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        var definition = CardDefinitionLoader.FromEmbeddedResource("sea-gate-reborn");
        var land = (Land)CardDefinitionFactory.Build(definition, owner);

        // CR 711 / 712 — attach the MDFC face tracker pre-flipped to the
        // back face (Sea Gate, Reborn is the back face that actually exists
        // on the battlefield).
    ...
    /// registered agent for the optional payment, and deducts 3 life on the
    /// yes path.
    /// </summary>
    ...
    private static bool TryPayThreeLifeOrEnterTapped(Player controller, ICard self)
    {
        ...
        try
        {
            wantsToPay = agent.ChooseYesNoAsync(
                question: "Pay 3 life so Sea Gate, Reborn enters untapped?",
