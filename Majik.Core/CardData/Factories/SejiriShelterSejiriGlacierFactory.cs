...
/// modal double-faced card <c>Sejiri Shelter // Sejiri Glacier</c> ({1}{W}).
///
/// Oracle text (verified against Scryfall):
///   Front — Sejiri Shelter, Instant {1}{W}:
///     "Target creature you control gains protection from the color of your
///      choice until end of turn."
///   Back — Sejiri Glacier, Land:
...
/// ("Sejiri Shelter // Sejiri Glacier"). The two individual faces are already
/// implemented by <see cref="SejiriShelterFactory"/> (front) and
/// <see cref="SejiriGlacierFactory"/> (back), but the engine's
...
/// <see cref="OnduInversionOnduSkyruinsFactory"/> and
...
/// combined name resolves to the FRONT face (Sejiri Shelter, an Instant). The
...
/// <c>MdfcCastFlow</c> can offer "play as the land Sejiri Glacier" with its
/// unconditional "this land enters tapped" ETB — CR 614.1c). This arm simply
/// delegates to <see cref="SejiriShelterFactory.Create"/> so the
/// cast-either-face wiring is defined in exactly one place.
/// </summary>
[CardName("Sejiri Shelter // Sejiri Glacier")]
public static class SejiriShelterSejiriGlacierFactory
{
    public const string CardName = "Sejiri Shelter // Sejiri Glacier";
    public const string Slug = "sejiri-shelter-sejiri-glacier";

    /// <summary>
    /// Build the combined-name MDFC as its castable FRONT face (Sejiri
    /// Shelter, Instant {1}{W}) with the <see cref="MdfcState"/> face tracker +
    ...
    /// protection-grant behaviour come from <see cref="SejiriShelterFactory"/>
    ...
    public static Instant Create(Player owner)
    {
        ...
        return SejiriShelterFactory.Create(owner);
    }
}
