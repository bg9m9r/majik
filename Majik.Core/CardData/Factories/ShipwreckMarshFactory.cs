...
/// <summary>
/// Named-card factory for Shipwreck Marsh (Innistrad: Midnight Hunt) — the
/// U/B member of the "slow land" cycle. Oracle (verified against Scryfall):
/// <code>
...
/// printed subtype) plus the two mana abilities {U}/{B} (CR 605.1 — mana
/// abilities don't use the stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/shipwreck-marsh.json</c> and materialized
...
/// is the slow-land ETB condition — the inverse direction of the fast-land
/// "two or fewer" wording. On the production load path it is wired
/// automatically from the printed oracle text by
/// <see cref="ConditionalEntersTappedBinder"/> (its
/// "N or [more|fewer] other lands" regex matches this exact wording with
/// direction="more", building the predicate
/// <c>CountOtherLands(controller, self) &gt;= 2</c>). This factory mirrors
/// that predicate on the optional <see cref="ReplacementBus"/> overload so
/// the behaviour is exercisable in isolation (same shape as
...
public static class ShipwreckMarshFactory
{
    ...
    /// <summary>
    /// Construct Shipwreck Marsh owned and controlled by
    ...
    /// <summary>
    /// Construct Shipwreck Marsh with an optional <see cref="ReplacementBus"/>
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        // on the production load path (direction="more").
        // ----------------------------------------------------------------
