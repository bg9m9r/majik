...
/// <summary>
/// Named-card factory for Silverbluff Bridge (The Brothers' War — the UR
...
/// </code>
///
/// Direct sibling of <see cref="RazortideBridgeFactory"/> — same artifact
/// "Bridge" cycle, same three clauses; only the produced colours differ
/// ({U}/{R} here vs {W}/{U} on Razortide Bridge).
///
/// ## Implemented
/// - <b>Artifact Land</b> (CR 301.1 / 305.1) — concrete <see cref="Land"/>
///   with <see cref="CardType.Artifact"/> additively flagged via
///   <see cref="Card.AddCardType"/> (mirrors Razortide Bridge / Darksteel
///   Citadel). Lands have no mana cost (CR 305.1). Not Basic; no land subtype.
/// - <b>Indestructible</b> (CR 702.12) — <see cref="KeywordAbility"/> marker,
///   read by the non-creature destroy gate.
...
/// - <b>{T}: Add {U} or {R}</b> (CR 605.1 — mana abilities don't use the
///   stack) — two vanilla <see cref="ManaAbility"/> instances, one per
///   produced colour.
/// </summary>
[CardName("Silverbluff Bridge")]
public static class SilverbluffBridgeFactory
{
    public const string CardName = "Silverbluff Bridge";

    /// <summary>
    /// Construct Silverbluff Bridge owned and controlled by
    ...
    /// <summary>
    /// Construct Silverbluff Bridge with an optional <see cref="ReplacementBus"/>.
    /// </summary>
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        // CR 301.1 / 305.1 — Silverbluff Bridge is an Artifact Land. The base
        // Land constructor only registers CardType.Land, so additively flag
        // the Artifact type (mirrors Razortide Bridge / Darksteel Citadel).
        // This is the gate the affinity-for-artifacts / metalcraft accounting
        // keys on.
        ...
        // ----------------------------------------------------------------
        // {T}: Add {U} or {R}. CR 605.1 — two mana abilities (no stack),
