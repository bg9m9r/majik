...
///   "{T}: Add {W}."
///
/// Front face — <see cref="SkyclaveClericFactory"/> (Creature — Kor Cleric
/// {1}{W} 1/3 with an ETB "you gain 2 life").
///
/// ## MDFC infra
///
/// See <see cref="SkyclaveClericFactory"/>'s class doc for the
/// cast-either-face design. This factory is the back-face dispatch arm: when
/// a player chooses to play the MDFC as a land,
/// <see cref="NamedCardFactory"/> resolves the back-face name
/// <c>"Skyclave Basilica"</c> and lands here. The card is constructed with
/// its <see cref="MdfcState"/> pre-flipped to the back face so the face
/// tracker reads as authoritative even though the back face is the permanent
/// that actually exists.
///
/// ## Card identity comes from JSON
///
/// Name / type and the <b>{T}: Add {W}</b> mana ability are loaded from the
/// embedded JSON definition (<c>skyclave-basilica.json</c>) via
...
///   <see cref="ReplacementBus"/>. Skyclave Basilica ALWAYS enters tapped —
///   no choice, no life payment — so the plain
...
/// ## References
///
/// - <see cref="AkoumTeethFactory"/> — sibling ZNR MDFC back-face tapland
///   that taps for a single color; this factory mirrors its MDFC /
///   mana-ability / unconditional enters-tapped wiring (swapping red for
///   white).
/// </summary>
[CardName("Skyclave Basilica")]
public static class SkyclaveBasilicaFactory
{
    public const string CardName = "Skyclave Basilica";
    public const string FrontName = "Skyclave Cleric";
    public const string Slug = "skyclave-basilica";

    /// <summary>
    /// Construct Skyclave Basilica without a <see cref="ReplacementBus"/>. The
    /// ETB "enters tapped" replacement is omitted; the {T}: Add {W} mana
    ...
    /// <summary>
    /// Construct Skyclave Basilica with an optional <see cref="ReplacementBus"/>
    /// for full ETB wiring.
    /// </summary>
