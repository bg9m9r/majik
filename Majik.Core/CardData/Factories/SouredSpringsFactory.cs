...
/// <summary>
/// Named-card factory for Soured Springs (Edge of Eternities "painland
/// Desert" cycle — the U/B member).
///
...
///    {T}: Add {U} or {B}."
///
/// Type line is <c>Land — Desert</c> (the printed Desert subtype). Same
/// shape as <see cref="AbradedBluffsFactory"/> (the R/W member), differing
/// only in the produced colours.
///
/// ## Implemented (v1)
/// - <b>Identity + dual mana</b> — loaded from
///   <c>Majik.Core/CardData/Cards/soured-springs.json</c> via
...
///   {U} and {B} (CR 605.1a — mana abilities don't use the stack). Same
///   dual-mana shape as the Refuge / Triome cycles.
...
///   text (this factory builds the land without it, matching the Abraded
///   Bluffs / Refuge / Temple cycle posture — the binder owns the
///   replacement so it isn't double-registered).
/// - <b>ETB triggered ability (CR 603.6a)</b> — "When this land enters,
///   it deals 1 damage to target opponent." Wired in code as a self-ETB
///   <see cref="TriggeredAbility"/> via
///   <see cref="Triggers.OnEnterBattlefieldSelf"/> with a 1..1 "target
///   opponent" <see cref="TargetRequest"/>. On resolution the chosen
///   opponent loses 1 life via <see cref="Fx.DealDamageAny"/> (Player →
///   <see cref="Player.LoseLife"/>) — exactly the Abraded Bluffs ETB-damage
///   wiring. CR 608.2b — no chosen target / illegal target at resolution →
///   clean no-op.
///
/// ## Deferred (v1 gaps)
...
///   description but not yet machine-enforced. Same posture as Abraded
///   Bluffs. No chosen target → the damage effect no-ops.
/// </summary>
[CardName("Soured Springs")]
public static class SouredSpringsFactory
{
    public const string CardName = "Soured Springs";
    public const int DamageAmount = 1;

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("soured-springs");

    /// <summary>
    /// Construct Soured Springs with no live wiring. The ETB trigger is
    ...
    /// shape-only path, matching the Abraded Bluffs / Refuge cycle posture.
    /// </summary>
    public static Land Create(Player owner) =>
        Create(owner, triggers: null);

    /// <summary>
    /// Construct Soured Springs. When <paramref name="triggers"/> is
    ...
    public static Land Create(Player owner, TriggerManager? triggers)
    {
        ...
        // Identity + dual {U}/{B} mana come from the JSON definition.
