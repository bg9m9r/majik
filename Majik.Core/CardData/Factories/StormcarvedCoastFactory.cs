...
/// <summary>
/// Named-card factory for Stormcarved Coast (Innistrad: Midnight Hunt) — the
/// U/R member of the "slow land" cycle. Oracle (verified against Scryfall):
/// <code>
...
/// printed subtype) plus the two mana abilities {U}/{R} (CR 605.1 — mana
/// abilities don't use the stack) — is declared declaratively in
/// <c>Majik.Core/CardData/Cards/stormcarved-coast.json</c> and materialized
...
/// <see cref="SeachromeCoastFactory"/>, only the direction differs — the slow
/// land enters untapped iff the controller controls at least two OTHER lands;
/// "other" excludes this land via reference equality; "you control" reads the
/// controller's battlefield only).
/// </para>
///
/// <para>
...
/// overload wires the predicate when the bus is supplied, and prod load wires
/// it from oracle text via the binder.
/// </para>
/// </summary>
...
public static class StormcarvedCoastFactory
{
    ...
    /// <summary>
    /// Construct Stormcarved Coast owned and controlled by
    ...
    /// <summary>
    /// Construct Stormcarved Coast with an optional
    ...
    public static Land Create(Player owner, ReplacementBus? replacements)
    {
        ...
        // Same shape ConditionalEntersTappedBinder emits from oracle text on
        // the production load path (direction "more" => >= threshold).
        // ----------------------------------------------------------------
