...
/// <summary>
/// Named-card factory for Temple of Enlightenment (Journey into Nyx — W/U
/// "scry land").
///
...
/// Nyx "Temple" scry-land cycle (e.g. Temple of Deceit {U}/{B}, Temple of
/// Triumph {R}/{W}) — only the two produced colours differ. Here they are
/// {W} and {U} (CR 605.1a — mana abilities don't use the stack).
/// </para>
///
/// <para>
/// ## Card identity + abilities come from JSON
/// Name / Land type, the two single-colour <see cref="Majik.Core.Abilities.ManaAbility"/>s
/// ({W} and {U}), and the "When this land enters, scry 1" ETB
/// <see cref="Majik.Core.Abilities.TriggeredAbility"/> are all declared in the
/// embedded JSON definition (<c>temple-of-enlightenment.json</c>) and
/// materialised via <see cref="CardDefinitionLoader.FromEmbeddedResource"/> +
...
/// the scry-land cycle (<see cref="TempleOfDeceitFactory"/>). The named
/// factory exists for the test / <see cref="NamedCardFactory"/> dispatch path
/// so unit tests get the mana + scry abilities without round-tripping through
/// the binder chain.
/// </para>
/// </summary>
...
{
    public const string CardName = "Temple of Enlightenment";

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("temple-of-enlightenment");

    /// <summary>
    /// Construct Temple of Enlightenment owned and controlled by
    /// <paramref name="owner"/>. Identity, the {W}/{U} mana abilities, and the
    /// ETB scry-1 trigger all come from the embedded JSON definition.
    /// </summary>
