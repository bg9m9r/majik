...
/// <summary>
/// Twisted Landscape — Modern Horizons 3 land.
///
...
/// - <b>{T}, Sacrifice this land: tutor a basic Swamp/Mountain/Forest onto the
...
/// - <b>Cycling {B}{R}{G}</b> (CR 702.32) — wired through the shared
///   <see cref="CyclingFactory.Build"/> primitive with cycle cost
///   <see cref="ManaCostCost"/>(<c>{B}{R}{G}</c>). When a bus is supplied the
...
/// </summary>
[CardName("Twisted Landscape")]
public static class TwistedLandscapeFactory
{
    public const string CardName = "Twisted Landscape";

    /// <summary>Construct Twisted Landscape owned and controlled by
    ...
    /// <summary>
    /// Construct Twisted Landscape owned and controlled by
    /// <paramref name="owner"/>.
    /// </summary>
    ...
    public static Land Create(Player owner, IEventBus? eventBus)
    {
        ...
        // {T}, Sacrifice this land: search for a basic Swamp/Mountain/Forest,
        ...
        // Cycling {B}{R}{G}. CR 702.32 — the primitive appends the
        // DiscardSelfCost hand-zone gate (CR 702.32a) and the CardCycledEvent
        // publish (CR 702.32d).
        CyclingFactory.Build(land, new ManaCostCost("BRG"), eventBus);
    ...
    /// subtype is Swamp, Mountain, or Forest (CR 205.4a — Basic supertype +
    ...
    private static async ValueTask TutorBasicSwampMountainForestToBattlefieldTappedAsync(Player player, ResolutionContext ctx)
    {
        ...
        // CR 701.20a — shuffle whether or not a card was found.
        LibraryShuffle.ShuffleLibrary(player, "twisted-landscape");
    }
}
