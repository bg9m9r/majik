...
/// <summary>
/// Named-card factory for Vivid Creek (Lorwyn "Vivid" land cycle).
///
...
/// for its base colour ({U}) with no counter cost. Two charge counters means
/// at most two off-colour pips over its lifetime; after that it remains a
/// mono-blue tapland. The exact cycle-mate of <see cref="VividMarshFactory"/>;
/// only the base colour differs ({B} -> {U}).
///
/// ## Implementation
///
/// Card identity (Land, no supertype / subtype) is loaded from
/// <c>Majik.Core/CardData/Cards/vivid-creek.json</c> through
...
/// ## {T}: Add {U} (CR 605.1)
///
/// One plain <see cref="ManaAbility"/> producing {U} with the standard
...
/// counter (distinguishing Vivid Creek from Sphere of the Suns, whose only
...
public static class VividCreekFactory
{
    public const string CardName = "Vivid Creek";
    public const int StartingChargeCounters = 2;

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("vivid-creek");

    /// <summary>
    /// Construct Vivid Creek with no live trigger-manager wiring. The ETB
    /// "two charge counters" trigger is attached for shape observability; the
    /// base {U} mana ability and the five WUBRG any-colour mana abilities are
    ...
    /// <summary>
    /// Construct Vivid Creek. When <paramref name="triggers"/> is supplied,
