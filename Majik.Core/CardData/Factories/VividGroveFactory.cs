...
/// <summary>
/// Named-card factory for Vivid Grove (Lorwyn "Vivid" land cycle).
///
...
/// for its base colour ({G}) with no counter cost. Two charge counters means
/// at most two off-colour pips over its lifetime; after that it remains a
/// mono-green tapland. The exact cycle-mate of <see cref="VividMarshFactory"/>;
/// only the base colour differs ({B} -> {G}).
///
/// ## Implementation
///
/// Card identity (Land, no supertype / subtype) is loaded from
/// <c>Majik.Core/CardData/Cards/vivid-grove.json</c> through
/// <see cref="CardDefinitionFactory"/>, matching the JSON-driven card cycle
/// (same posture as <see cref="VividMarshFactory"/> / <see cref="SphereOfTheSunsFactory"/>).
///
...
/// counters at battlefield entry — same shape <see cref="VividMarshFactory"/>
/// / Sphere of the Suns / Blast Zone use. The strict CR 614.1d "enters with N
/// counters" replacement only carries the +1/+1 channel today
/// (<see cref="Majik.Core.Effects.EntersWithCountersReplacement"/>), so the
/// trigger-shape is used for charge counters. The trigger registers with a
/// live <see cref="TriggerManager"/> when one is supplied.
///
/// ## {T}: Add {G} (CR 605.1)
///
/// One plain <see cref="ManaAbility"/> producing {G} with the standard
...
/// counter (distinguishing Vivid Grove from Sphere of the Suns, whose only
...
/// oracle text — same posture as <see cref="VividMarshFactory"/> /
...
/// </summary>
[CardName("Vivid Grove")]
public static class VividGroveFactory
{
    public const string CardName = "Vivid Grove";
    public const int StartingChargeCounters = 2;

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("vivid-grove");

    /// <summary>
    /// Construct Vivid Grove with no live trigger-manager wiring. The ETB
    /// "two charge counters" trigger is attached for shape observability; the
    /// base {G} mana ability and the five WUBRG any-colour mana abilities are
    ...
    /// <summary>
    /// Construct Vivid Grove. When <paramref name="triggers"/> is supplied,
    ...
    public static Land Create(Player owner, TriggerManager? triggers)
    {
        ...
        // posture as Vivid Marsh / Sphere of the Suns / Blast Zone.
        // ----------------------------------------------------------------
