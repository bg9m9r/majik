...
/// <summary>
/// Named-card factory for Vivid Marsh (Lorwyn "Vivid" land cycle).
///
...
/// for its base colour ({B}) with no counter cost. Two charge counters means
/// at most two off-colour pips over its lifetime; after that it remains a
/// mono-black tapland. The exact cycle-mate of <see cref="VividCragFactory"/>;
/// only the base colour differs ({R} -> {B}).
///
/// ## Implementation
///
/// Card identity (Land, no supertype / subtype) is loaded from
/// <c>Majik.Core/CardData/Cards/vivid-marsh.json</c> through
/// <see cref="CardDefinitionFactory"/>, matching the JSON-driven card cycle
/// (same posture as <see cref="VividCragFactory"/> / <see cref="SphereOfTheSunsFactory"/>).
///
...
/// counters at battlefield entry — same shape <see cref="VividCragFactory"/>
...
/// ## {T}: Add {B} (CR 605.1)
///
/// One plain <see cref="ManaAbility"/> producing {B} with the standard
...
/// counter (distinguishing Vivid Marsh from Sphere of the Suns, whose only
...
/// oracle text — same posture as <see cref="VividCragFactory"/> /
...
public static class VividMarshFactory
{
    public const string CardName = "Vivid Marsh";
    public const int StartingChargeCounters = 2;

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("vivid-marsh");

    /// <summary>
    /// Construct Vivid Marsh with no live trigger-manager wiring. The ETB
    /// "two charge counters" trigger is attached for shape observability; the
    /// base {B} mana ability and the five WUBRG any-colour mana abilities are
    ...
    /// <summary>
    /// Construct Vivid Marsh. When <paramref name="triggers"/> is supplied,
    ...
    public static Land Create(Player owner, TriggerManager? triggers)
    {
        ...
        // posture as Vivid Crag / Sphere of the Suns / Blast Zone.
        // ----------------------------------------------------------------
