...
/// <summary>
/// Named-card factory for Vivid Meadow (Lorwyn "Vivid" land cycle).
///
...
/// for its base colour ({W}) with no counter cost. Two charge counters means
/// at most two off-colour pips over its lifetime; after that it remains a
/// mono-white tapland. The exact cycle-mate of <see cref="VividCragFactory"/>
/// (base {W} instead of {R}).
///
/// ## Implementation
///
/// Card identity (Land, no supertype / subtype) is loaded from
/// <c>Majik.Core/CardData/Cards/vivid-meadow.json</c> through
...
/// / Sphere of the Suns use. The strict CR 614.1d "enters with N counters"
...
/// ## {T}: Add {W} (CR 605.1)
///
/// One plain <see cref="ManaAbility"/> producing {W} with the standard
...
/// counter (distinguishing it from Sphere of the Suns, whose only mana
/// ability is the counter-gated any-colour one).
///
...
/// oracle text — same posture as <see cref="VividCragFactory"/> / the Refuge
/// / Temple cycle. This factory builds the land untapped for test
/// convenience (callers that need the live ETB-tapped behaviour drive it
/// through the binder chain).
/// </summary>
[CardName("Vivid Meadow")]
public static class VividMeadowFactory
{
    public const string CardName = "Vivid Meadow";
    public const int StartingChargeCounters = 2;

    private static readonly CardDefinition Definition =
        CardDefinitionLoader.FromEmbeddedResource("vivid-meadow");

    /// <summary>
    /// Construct Vivid Meadow with no live trigger-manager wiring. The ETB
    /// "two charge counters" trigger is attached for shape observability; the
    /// base {W} mana ability and the five WUBRG any-colour mana abilities are
    ...
    /// <summary>
    /// Construct Vivid Meadow. When <paramref name="triggers"/> is supplied,
    ...
    public static Land Create(Player owner, TriggerManager? triggers)
    {
        ...
        // posture as Vivid Crag / Sphere of the Suns.
        // ----------------------------------------------------------------
