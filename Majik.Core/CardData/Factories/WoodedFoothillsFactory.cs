...
/// <summary>
/// Named-card factory for Wooded Foothills (Onslaught / reprints).
///
/// Land. Oracle text:
///   "{T}, Pay 1 life, Sacrifice Wooded Foothills: Search your library for a
///    Mountain or Forest card, put it onto the battlefield, then shuffle."
///
/// ## Implementation (v1)
/// Same shape as <see cref="PollutedDeltaFactory"/>. Fetches
/// <see cref="CardSubtype.Mountain"/> or <see cref="CardSubtype.Forest"/>.
///
/// ## Deferred (v1 gaps)
/// Library shuffle (CR 701.19c) and agent-driven pick deferred — same as
/// Polluted Delta and all other tutors in the codebase.
/// </summary>
public static class WoodedFoothillsFactory
{
    public const string CardName = "Wooded Foothills";

    /// <summary>
    /// Construct Wooded Foothills owned and controlled by <paramref name="owner"/>.
    /// </summary>
    public static Land Create(Player owner)
    {
        ...
        var fetchAbility = new ActivatedAbility(
            ...
            {
                new Effect(
                    "Wooded Foothills: search library for Mountain or Forest, put onto battlefield",
                    () => FetchLandEffect(owner, land, CardSubtype.Mountain, CardSubtype.Forest)),
            });
