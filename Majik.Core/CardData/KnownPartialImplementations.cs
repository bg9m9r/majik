    ...
    public static readonly IReadOnlyDictionary<string, CardGap> ByName =
        new Dictionary<string, CardGap>(StringComparer.Ordinal)
        {
            ["Agatha's Soul Cauldron"] = new CardGap(
                ...
                + "reconstructing firebreathing / self-keyword-grant / self-counter / regenerate-self "
                + "/ draw-a-card / gain-life / pinger / sacrifice-self-pinger "
                + "from oracle text (self-pump now covers SIGNED deltas — a negative power/toughness "
                + "leg as on Aetherling / Canyon Crab / Flowstone — and the cost grammar accepts "
                + "concatenated multi-symbol mana like '{1}{U}', not just comma-separated single pips). "
