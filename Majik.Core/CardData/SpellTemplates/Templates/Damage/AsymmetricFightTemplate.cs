...
public sealed class AsymmetricFightTemplate : ISpellTemplate
{
    ...
    private static readonly Regex Pattern = new(
        @"^target\s+creature\s+(?:you\s+control|an\s+opponent\s+controls)\s+deals\s+damage\s+equal\s+to\s+its\s+power\s+to\s+(?:another\s+|any\s+)?target(?:\s+(?<targetKind>[a-z][a-z0-9\s,'\-]{0,80}?))?\.",
