...
///
///   "You may sacrifice a nontoken green creature rather than pay this
///    spell's mana cost."
///
/// Oracle pattern used by Flare of Cultivation (MH3). The caster chooses one
/// nontoken green creature they control on the battlefield and sacrifices it
/// instead of paying the spell's printed mana cost. Recolored sibling of
/// <see cref="SacrificeNontokenBlueCreatureAlternativeCost"/> (Flare of Denial)
/// — same shape, green identity. Like the rest of the Flare cycle there is no
/// timing restriction ("not your turn") — this alternative cost is available
/// whenever the spell is otherwise castable.
///
/// Post-resolution (<see cref="OnResolved"/>): the chosen creature moves
/// Battlefield → Graveyard (sacrifice, CR 701.18).
/// </summary>
public sealed class SacrificeNontokenGreenCreatureAlternativeCost : IAlternativeCost
{
    /// <summary>The nontoken green creature the caster chose to sacrifice.</summary>
    public Permanent SacrificedCreature { get; }

    /// <inheritdoc/>
    public string Description =>
        $"Sacrifice nontoken green creature ({SacrificedCreature.Name}) instead of paying mana cost";
    ...
    public SacrificeNontokenGreenCreatureAlternativeCost(Permanent sacrificedCreature)
    {
    ...
    /// nontoken green creature, be controlled by the caster (CR 701.18a),
    ...
    public bool CanCastFor(ICard card, Player caster)
    {
        ...
        if (!CardColors.GetColors(SacrificedCreature).Contains(ManaColor.Green)) return false;
