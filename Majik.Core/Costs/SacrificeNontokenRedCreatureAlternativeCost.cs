...
/// <summary>
/// CR 118.9 — "Flare" alternative cost (red variant).
///
///   "You may sacrifice a nontoken red creature rather than pay this
///    spell's mana cost."
///
/// Oracle pattern used by Flare of Duplication (MH3). The caster chooses one
/// nontoken red creature they control on the battlefield and sacrifices it
/// instead of paying the spell's printed mana cost. As with the blue
/// <see cref="SacrificeNontokenBlueCreatureAlternativeCost"/> (Flare of
/// Denial) there is no timing restriction — this alternative cost is available
...
/// </summary>
public sealed class SacrificeNontokenRedCreatureAlternativeCost : IAlternativeCost
{
    /// <summary>The nontoken red creature the caster chose to sacrifice.</summary>
    public Permanent SacrificedCreature { get; }

    /// <inheritdoc/>
    public string Description =>
        $"Sacrifice nontoken red creature ({SacrificedCreature.Name}) instead of paying mana cost";
    ...
    public SacrificeNontokenRedCreatureAlternativeCost(Permanent sacrificedCreature)
    {
    ...
    /// nontoken red creature, and be controlled by the caster (CR 701.18a).
    /// </summary>
    public bool CanCastFor(ICard card, Player caster)
    {
        ...
        if (!CardColors.GetColors(SacrificedCreature).Contains(ManaColor.Red)) return false;
