...
/// <summary>
/// Cursed Totem — printed static (CR 602.5c / 605):
/// "Activated abilities of creatures can't be activated unless they're
/// mana abilities."
///
/// Lifecycle binder analogous to <see cref="StonySilenceStaticEffect"/>,
/// but the predicate matches creature sources instead of artifact
/// sources. Symmetric global — both controllers' creatures are gated,
/// matching the printed text (no "your opponents control" qualifier).
///
...
/// before the predicate runs — Cursed Totem's printed text is consistent
...
/// entirely, so Birds of Paradise / Llanowar Elves tap-for-mana abilities
/// keep working under Cursed Totem.
/// </summary>
public sealed class CursedTotemStaticEffect
{
    ...
    public CursedTotemStaticEffect(Permanent? source, IEventBus? eventBus)
    {
        _source = source;
        _eventBus = eventBus;
        _handler = OnEvent;
    ...
    /// a creature currently on the battlefield (CR 602.2 — only on-
    ...
    /// creatures; mana-ability exemption is applied by the registry
    /// itself before this predicate runs.
    /// </summary>
    private bool IsBlocked(IActivatedAbility ability)
    {
        // Defensive — mirror StonySilenceStaticEffect's self-deactivation
        // guard. If the lifecycle binder failed to unregister on LTB (e.g.
        // xUnit parallel-tests leaking state), honour the printed "while
        // on the battlefield" implicit scope (CR 604.2) at consult time.
        ...
        // Only on-battlefield creatures — Cursed Totem's printed static
        // targets creature permanents, not "creature cards" in any zone.
