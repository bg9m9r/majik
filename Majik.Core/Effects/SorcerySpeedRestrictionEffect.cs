...
/// <summary>
/// Reusable lifecycle binder for a "target players can cast spells only any
/// time they could cast a sorcery" static effect (CR 117.1a — Teferi, Time
/// Raveler and friends; CR 114 — emblem-with-static surface).
///
/// While the source permanent is on the battlefield (or, for an emblem, for
/// the rest of the game), a sorcery-speed restriction is registered against
/// each player produced by the supplied <c>affectedPlayersResolver</c>. When
/// the source leaves the battlefield, all entries keyed by this lifecycle
/// instance are removed.
///
/// Lifecycle mirrors <see cref="RetypeLandsStaticEffect"/>:
/// <list type="bullet">
...
/// support, pass <c>permanentSource: null</c> + <c>permanentScope: () =>
/// true</c>. The effect then registers immediately on Attach and never
/// detaches by zone event (emblems last for the rest of the game per CR
/// 114). Attaching is still required so the registry actually picks up
/// the restriction.
/// </summary>
public sealed class SorcerySpeedRestrictionEffect
{
    ...
    /// <summary>
    /// Build a sorcery-speed-restriction lifecycle.
    /// </summary>
    ...
    /// opponents added/removed mid-game (multiplayer) are picked up
    ...
    /// otherwise.</param>
    public SorcerySpeedRestrictionEffect(
        Permanent? source,
        IEventBus? eventBus,
    ...
    private void Sync()
    {
        if (_activeWhile())
        {
            ...
            {
                if (p == null) continue;
                CastingRestrictions.AddSorcerySpeedRestriction(_token, p);
            }
            _registered = true;
        }
        else
    ...
    {
        if (!_registered) return;
        CastingRestrictions.RemoveSorcerySpeedRestriction(_token);
        _registered = false;
    }
}
